...
struct TransducerKeywordResult {
  int32_t frame_offset = 0;  // Number of frames after subsampling we have decoded so far
  std::vector<int64_t> tokens;  // The decoded token IDs for keywords
  std::string keyword;  // The triggered keyword
  int32_t num_trailing_blanks = 0;  // Number of trailing blank frames decoded so far
  std::vector<int32_t> timestamps;  // timestamps[i] contains the output frame index where tokens[i] is decoded.
  Hypotheses hyps;  // Used only in modified beam_search
};
...
class TransducerKeywordDecoder {
 ...
 private:
  ...
  int32_t unk_id_;
  std::mutex decode_mutex_;  // Mutex for thread safety
};
...
//By GST ARMV8 GCC13.2@2024/10/23