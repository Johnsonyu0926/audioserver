...
Ort::Value Transpose12(OrtAllocator *allocator, const Ort::Value *v) {
  ...
  const std::array<int64_t, 3> ans_shape{shape[0], shape[2], shape[1]};
  Ort::Value ans = Ort::Value::CreateTensor<T>(allocator, ans_shape.data(), ans_shape.size());
  T *dst = ans.GetTensorMutableData<T>();
  const auto row_stride = shape[2];
...
template Ort::Value Transpose01<float>(OrtAllocator *allocator, const Ort::Value *v);
template Ort::Value Transpose12<float>(OrtAllocator *allocator, const Ort::Value *v);
