// File: source/router/sherpa-onnx/sherpa-onnx/csrc/context-graph-test.cc
...
static void TestHelper(const std::map<std::string, float> &queries, float score,
  ...
  for (const auto &context : contexts_str) {
    contexts.emplace_back(context.begin(), context.end());
    scores.push_back(std::round(score / context.size() * 100) / 100);
  }
