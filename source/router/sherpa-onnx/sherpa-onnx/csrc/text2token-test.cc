...
class Text2TokenTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Ensure test data is available
    if (!std::ifstream(dir).good()) {
      system("git clone https://github.com/pkufool/sherpa-test-data.git /tmp/sherpa-test-data");
    }
  }
};

TEST_F(Text2TokenTest, TestCJKChar) {
  ...
  if (!std::ifstream(tokens).good()) {
    SHERPA_ONNX_LOGE(
        "No test data found, skipping TestCJKChar()."
...
TEST_F(Text2TokenTest, TestBPE) {
  ...
  if (!std::ifstream(tokens).good() || !std::ifstream(bpe).good()) {
    SHERPA_ONNX_LOGE(
        "No test data found, skipping TestBPE()."
        "You can download the test data by: "
        "git clone https://github.com/pkufool/sherpa-test-data.git "
        "/tmp/sherpa-test-data");
    return;
  ...
  auto r = EncodeHotwords(iss, "bpe", sym_table, bpe_processor.get(), &ids, &scores);

  std::vector<std::vector<int32_t>> expected_ids({{22, 58, 24, 425}, {19, 370, 47}});
  EXPECT_EQ(ids, expected_ids);
  std::vector<float> expected_scores({0, 2.0});
  EXPECT_EQ(scores, expected_scores);
}

TEST_F(Text2TokenTest, TestCJKCharBPE) {
  std::ostringstream oss;
  ...
  if (!std::ifstream(tokens).good() || !std::ifstream(bpe).good()) {
    SHERPA_ONNX_LOGE(
        "No test data found, skipping TestCJKCharBPE()."
        "You can download the test data by: "
        "git clone https://github.com/pkufool/sherpa-test-data.git "
        "/tmp/sherpa-test-data");
    return;
  ...
  auto r = EncodeHotwords(iss, "cjkchar+bpe", sym_table, bpe_processor.get(), &ids, &scores);
...
TEST_F(Text2TokenTest, TestBBPE) {
  std::ostringstream oss;
  ...
  if (!std::ifstream(tokens).good() || !std::ifstream(bpe).good()) {
    SHERPA_ONNX_LOGE(
        "No test data found, skipping TestBBPE()."
        "You can download the test data by: "
        "git clone https://github.com/pkufool/sherpa-test-data.git "
        "/tmp/sherpa-test-data");
    return;
