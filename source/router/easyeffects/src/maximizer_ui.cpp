...
void setup(MaximizerBox* self, std::shared_ptr<Maximizer> maximizer, const std::string& schema_path) {
  ...
  self->data->connections.push_back(maximizer->reduction.connect([=](const double value) {
    ...
    util::idle_add(
        [=]() {
          if (get_ignore_filter_idle_add(serial)) return;
          if (!GTK_IS_LEVEL_BAR(self->reduction_levelbar) || !GTK_IS_LABEL(self->reduction_label)) return;
  ...
  g_settings_bind(self->settings, "release", gtk_spin_button_get_adjustment(self->release), "value", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(self->settings, "threshold", gtk_spin_button_get_adjustment(self->threshold), "value", G_SETTINGS_BIND_DEFAULT);
}
