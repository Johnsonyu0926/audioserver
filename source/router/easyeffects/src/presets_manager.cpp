...
try {
  while (it != std::filesystem::directory_iterator{}) {
    if (std::filesystem::is_regular_file(it->status()) && it->path().extension() == json_ext) {
      cp_paths.emplace_back(origin + "/" + it->path().stem().string());
      ...
      if (auto path = it->path(); !path.empty()) {
        ...
        const auto sub_cp_vect = scan_community_package_recursive(subdir_it, scan_level, origin + "/" + path.filename().string());
        cp_paths.insert(cp_paths.end(), sub_cp_vect.cbegin(), sub_cp_vect.cend());
      }
    }
    ++it;
...
void PresetsManager::save_blocklist(const PresetType& preset_type, nlohmann::json& json) {
  ...
  const auto list = (preset_type == PresetType::output)
                        ? util::gchar_array_to_vector(g_settings_get_strv(soe_settings, "blocklist"))
                        : util::gchar_array_to_vector(g_settings_get_strv(sie_settings, "blocklist"));

  for (const auto& l : list) {
    blocklist.push_back(l);
  }

  json[(preset_type == PresetType::output) ? "output" : "input"]["blocklist"] = blocklist;
}

auto PresetsManager::load_blocklist(const PresetType& preset_type, const nlohmann::json& json) -> bool {
  try {
    auto list = json.at((preset_type == PresetType::output) ? "output" : "input").at("blocklist").get<std::vector<std::string>>();
    g_settings_set_strv((preset_type == PresetType::output) ? soe_settings : sie_settings, "blocklist", util::make_gchar_pointer_vector(list).data());
  } catch (const nlohmann::json::exception& e) {
    g_settings_reset((preset_type == PresetType::output) ? soe_settings : sie_settings, "blocklist");
...
void PresetsManager::save_preset_file(const PresetType& preset_type, const std::string& name) {
  ...
  const auto plugins = util::gchar_array_to_vector(g_settings_get_strv((preset_type == PresetType::output) ? soe_settings : sie_settings, "plugins"));
  ...
  json[(preset_type == PresetType::output) ? "output" : "input"]["plugins_order"] = list;
  write_plugins_preset(preset_type, plugins, json);
  output_file = ((preset_type == PresetType::output) ? user_output_dir : user_input_dir) / std::filesystem::path{name + json_ext};
...
void PresetsManager::remove(const PresetType& preset_type, const std::string& name) {
  const auto conf_dir = (preset_type == PresetType::output) ? user_output_dir : user_input_dir;
  const auto preset_file = conf_dir / std::filesystem::path{name + json_ext};
