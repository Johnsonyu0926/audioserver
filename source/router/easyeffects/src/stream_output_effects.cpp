// Filename: stream_output_effects.cpp
...
void StreamOutputEffects::connect_filters(const bool& bypass) {
  ...
  while (pm->count_node_ports(pm->output_device.id) < 2) {
    ...
    if (timeout > 10000) {
      util::warning("Information about the ports of the output device " + pm->output_device.name + " with id " + util::to_string(pm->output_device.id) + " are taking to long to be available. Aborting the link");
      return;
    }
  }
  ...
  if (links.size() < 2U) {
    util::warning(" link from node " + util::to_string(prev_node_id) + " to output device " + util::to_string(next_node_id) + " failed");
  }
}
