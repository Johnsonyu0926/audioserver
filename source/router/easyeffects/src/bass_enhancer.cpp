...
BassEnhancer::BassEnhancer(const std::string& tag,
    ...
    : PluginBase(tag,
                 ...
                 pipe_manager,
                 pipe_type) {
  lv2_wrapper = std::make_unique<lv2::Lv2Wrapper>("http://drobilla.net/plugins/mda/BassEnhancer");

  package_installed = lv2_wrapper->found_plugin;

  if (!package_installed) {
    util::debug(log_tag + "http://drobilla.net/plugins/mda/BassEnhancer is not installed");
  }

  lv2_wrapper->bind_key_double_db<"enhance", "enhance">(settings);
