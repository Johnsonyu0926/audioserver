...
void setup(BassEnhancerBox* self, std::shared_ptr<BassEnhancer> bass_enhancer, const std::string& schema_path) {
  ...
  auto connect_level = [&](auto& connection, auto signal, auto update_func) {
    connection = signal.connect([=](const auto&... args) {
      g_object_ref(self);
      util::idle_add(
          [=]() {
            if (get_ignore_filter_idle_add(serial)) return;
            update_func(args...);
          },
          [=]() { g_object_unref(self); });
    });
  ...
  connect_level(self->data->connections.emplace_back(), bass_enhancer->input_level, [&](const float left, const float right) {
  ...
  connect_level(self->data->connections.emplace_back(), bass_enhancer->output_level, [&](const float left, const float right) {
  ...
  connect_level(self->data->connections.emplace_back(), bass_enhancer->harmonics, [&](const double value) {
    if (!GTK_IS_LEVEL_BAR(self->harmonics_levelbar) || !GTK_IS_LABEL(self->harmonics_levelbar_label)) return;
  ...
  gsettings_bind_widgets<"amount", "harmonics", "scope", "floor", "blend", "listen", "floor-active">(self->settings, self->amount, self->harmonics, self->scope, self->floor, self->blend, self->listen, self->floor_active);
}
...
void dispose(GObject* object) {
  ...
  for (auto& handler_id : self->data->gconnections) g_signal_handler_disconnect(self->settings, handler_id);
...
void bass_enhancer_box_class_init(BassEnhancerBoxClass* klass) {
  ...
  gtk_widget_class_bind_template_child(widget_class, input_level_right);
  gtk_widget_class_bind_template_child(widget_class, output_level_left);
  gtk_widget_class_bind_template_child(widget_class, output_level_right);
  gtk_widget_class_bind_template_child(widget_class, input_level_left_label);
  gtk_widget_class_bind_template_child(widget_class, input_level_right_label);
  gtk_widget_class_bind_template_child(widget_class, output_level_left_label);
  gtk_widget_class_bind_template_child(widget_class, output_level_right_label);
  gtk_widget_class_bind_template_child(widget_class, plugin_credit);
  gtk_widget_class_bind_template_child(widget_class, harmonics_levelbar_label);
  gtk_widget_class_bind_template_child(widget_class, harmonics_levelbar);
  gtk_widget_class_bind_template_child(widget_class, floor);
  gtk_widget_class_bind_template_child(widget_class, amount);
  gtk_widget_class_bind_template_child(widget_class, harmonics);
  gtk_widget_class_bind_template_child(widget_class, scope);
  gtk_widget_class_bind_template_child(widget_class, blend);
  gtk_widget_class_bind_template_child(widget_class, floor_active);
  gtk_widget_class_bind_template_child(widget_class, listen);
