...
void setup(DeesserBox* self, std::shared_ptr<Deesser> deesser, const std::string& schema_path) {
  ...
  auto connect_signal = [&](auto& signal, auto&& callback) {
    self->data->connections.push_back(signal.connect([=](auto&&... args) {
      g_object_ref(self);
      util::idle_add(
          [=]() {
  ...
  connect_signal(deesser->input_level, [&](const float left, const float right) {
  ...
  connect_signal(deesser->output_level, [&](const float left, const float right) {
  ...
  connect_signal(deesser->detected, [&](const double value) {
  ...
  connect_signal(deesser->compression, [&](const double value) {
  ...
  auto bind_setting = [&](const char* key, auto* widget, const char* property) {
    g_settings_bind(self->settings, key, widget, property, G_SETTINGS_BIND_DEFAULT);
  };

  bind_setting("makeup", gtk_spin_button_get_adjustment(self->makeup), "value");
  bind_setting("ratio", gtk_spin_button_get_adjustment(self->ratio), "value");
  bind_setting("threshold", gtk_spin_button_get_adjustment(self->threshold), "value");
  bind_setting("f1-freq", gtk_spin_button_get_adjustment(self->f1_freq), "value");
  bind_setting("f2-freq", gtk_spin_button_get_adjustment(self->f2_freq), "value");
  bind_setting("f1-level", gtk_spin_button_get_adjustment(self->f1_level), "value");
  bind_setting("f2-level", gtk_spin_button_get_adjustment(self->f2_level), "value");
  bind_setting("f2-q", gtk_spin_button_get_adjustment(self->f2_q), "value");
  bind_setting("laxity", gtk_spin_button_get_adjustment(self->laxity), "value");
  bind_setting("sc-listen", self->sc_listen, "active");
