...
void setup(GateBox* self, std::shared_ptr<Gate> gate, const std::string& schema_path, PipeManager* pm) {
  ...
  auto connect_signal = [&](auto& signal, auto callback) {
  ...
  bind_spin_button("dry", self->dry);
  bind_spin_button("wet", self->wet);
  bind_spin_button("attack", self->attack);
  bind_spin_button("release", self->release);
  bind_spin_button("curve-threshold", self->curve_threshold);
  bind_spin_button("curve-zone", self->curve_zone);
  bind_spin_button("hysteresis-threshold", self->hysteresis_threshold);
  bind_spin_button("hysteresis-zone", self->hysteresis_zone);
  bind_spin_button("reduction", self->reduction);
  bind_spin_button("makeup", self->makeup);
  bind_spin_button("sidechain-preamp", self->preamp);
  bind_spin_button("sidechain-reactivity", self->reactivity);
  bind_spin_button("sidechain-lookahead", self->lookahead);
  bind_spin_button("hpf-frequency", self->hpf_freq);
  bind_spin_button("lpf-frequency", self->lpf_freq);
