...
struct _SpeexBox {
  ...
  GtkLabel *input_level_left_label, *input_level_right_label, *output_level_left_label, *output_level_right_label, *noise_suppression_label, *plugin_credit;
...
void setup(SpeexBox* self,
  ...
  auto connect_signal = [self](auto signal, auto callback) {
    self->data->connections.push_back(signal.connect(callback));
  };

  connect_signal(speex->input_level, [=](const float left, const float right) {
    g_object_ref(self);
    util::idle_add(
        [=]() {
  ...
  connect_signal(speex->output_level, [=](const float left, const float right) {
    g_object_ref(self);
    util::idle_add(
        [=]() {
          if (get_ignore_filter_idle_add(serial)) {
