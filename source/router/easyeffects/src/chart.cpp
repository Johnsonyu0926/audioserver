...
struct Data {
 public:
  ...
  bool draw_bar_border = true, fill_bars = true, is_visible = true, rounded_corners = false, dynamic_y_scale = true;
  int x_axis_height = 0, n_x_decimals = 1, n_y_decimals = 1;
  double mouse_y = 0.0, mouse_x = 0.0, margin = 0.02, line_width = 2.0;
  ...
  ChartScale chart_scale = ChartScale::logarithmic;
  GdkRGBA background_color = {0.0F, 0.0F, 0.0F, 1.0F}, color = {1.0F, 1.0F, 1.0F, 1.0F}, color_axis_labels = {1.0F, 1.0F, 1.0F, 1.0F}, gradient_color = {1.0F, 1.0F, 1.0F, 1.0F};
...
auto get_is_visible(Chart* self) -> bool {
  return GTK_IS_WIDGET(self) && self->data && self->data->is_visible;
}
...
void set_x_data(Chart* self, const std::vector<double>& x) {
  if (!GTK_IS_WIDGET(self) || x.empty() || !self->data) {
    return;
  }
  ...
  std::ranges::for_each(self->data->x_axis, [&](auto& v) { v = (v - self->data->x_min) / (self->data->x_max - self->data->x_min); });
  std::ranges::for_each(self->data->x_axis_log, [&](auto& v) { v = (v - self->data->x_min_log) / (self->data->x_max_log - self->data->x_min_log); });
}

void set_y_data(Chart* self, const std::vector<double>& y) {
  if (!GTK_IS_WIDGET(self) || y.empty() || !self->data) {
    return;
  }
  ...
  if (self->data->dynamic_y_scale) {
    ...
    self->data->global_min_y = std::min(min_y, self->data->global_min_y);
    self->data->global_max_y = std::max(max_y, self->data->global_max_y);
  ...
  if (std::fabs(self->data->y_max - self->data->y_min) < 0.00001) {
    ...
    std::ranges::for_each(self->data->y_axis, [&](auto& v) { v = (v - self->data->y_min) / (self->data->y_max - self->data->y_min); });
  }
...
void on_pointer_motion(GtkEventControllerMotion* controller, double xpos, double ypos, Chart* self) {
  ...
  if (y < height - self->data->x_axis_height && y > self->data->margin * height && x > self->data->margin * width &&
    ...
    self->data->mouse_y = (usable_height - y) / usable_height * (self->data->y_max - self->data->y_min) + self->data->y_min;
    ...
    switch (self->data->chart_scale) {
      case ChartScale::logarithmic: {
        const double mouse_x_log = (x - self->data->margin * width) / (width - 2 * self->data->margin * width) *
                                   ...
                                   self->data->x_min_log;
        self->data->mouse_x = std::pow(10.0, mouse_x_log);
        break;
      }
...
void snapshot(GtkWidget* widget, GtkSnapshot* snapshot) {
  ...
  if (const auto n_points = self->data->y_axis.size(); n_points > 0) {
    ...
    std::array<float, 4> border_width = {static_cast<float>(self->data->line_width), static_cast<float>(self->data->line_width), static_cast<float>(self->data->line_width), static_cast<float>(self->data->line_width)};
    ...
    switch (self->data->chart_type) {
      ...
      case ChartType::line: {
        ...
        for (uint n = 0U; n < n_points - 1U; n++) {
          ...
          cairo_line_to(ctx, self->data->objects_x[n + 1U], self->data->margin * height + usable_height - next_point_height);
        }
...
void chart_init(Chart* self) {
  ...
  g_signal_connect(GTK_WIDGET(self), "hide", G_CALLBACK(+[](GtkWidget* widget, Chart* self) { self->data->is_visible = false; }), self);
  g_signal_connect(GTK_WIDGET(self), "show", G_CALLBACK(+[](GtkWidget* widget, Chart* self) { self->data->is_visible = true; }), self);
