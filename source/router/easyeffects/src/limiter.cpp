...
Limiter::Limiter(const std::string& tag,
  ...
  auto connect_signal = [this](const char* key, GCallback callback) {
    gconnections.push_back(g_signal_connect(settings, key, callback, this));
  };

  connect_signal("changed::external-sidechain", G_CALLBACK(+[](GSettings* settings, char* key, gpointer user_data) {
    static_cast<Limiter*>(user_data)->update_sidechain_links(key);
  }));

  connect_signal("changed::sidechain-input-device", G_CALLBACK(+[](GSettings* settings, char* key, gpointer user_data) {
    static_cast<Limiter*>(user_data)->update_sidechain_links(key);
  }));
...
void Limiter::update_sidechain_links(const std::string& key) {
  if (!g_settings_get_boolean(settings, "external-sidechain")) {
