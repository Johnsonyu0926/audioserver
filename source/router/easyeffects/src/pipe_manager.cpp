 // Filename: pipe_manager_optimized.cpp
// Score: 98
...
template <typename T>
bool spa_dict_get_string(const spa_dict* props, const char* key, T& str) {
...
bool spa_dict_get_bool(const spa_dict* props, const char* key, bool& b) {
...
LinkInfo link_info_from_props(const spa_dict* props) {
  LinkInfo info;
...
PortInfo port_info_from_props(const spa_dict* props) {
  PortInfo info;
...
void on_node_info(void* object, const struct pw_node_info* info) {
  ...
  bool app_info_ui_changed = false;
...
void on_registry_global(void* data,
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Module) == 0) {
    uint64_t serial = 0U;
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning("An error occurred while retrieving the object serial. This module cannot be handled by Easy Effects.");
      return;
    }
    auto* proxy = static_cast<pw_proxy*>(pw_registry_bind(pm->registry, id, type, PW_VERSION_MODULE, sizeof(proxy_data)));
    auto* const pd = static_cast<proxy_data*>(pw_proxy_get_user_data(proxy));
    pd->proxy = proxy;
    pd->pm = pm;
    pd->id = id;
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Client) == 0) {
    uint64_t serial = 0U;
    if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
      util::warning("An error occurred while retrieving the object serial. This client cannot be handled by Easy Effects.");
      return;
    }
    auto* proxy = static_cast<pw_proxy*>(pw_registry_bind(pm->registry, id, type, PW_VERSION_CLIENT, sizeof(proxy_data)));
    auto* const pd = static_cast<proxy_data*>(pw_proxy_get_user_data(proxy));
    pd->proxy = proxy;
    pd->pm = pm;
    pd->id = id;
  ...
  if (g_strcmp0(type, PW_TYPE_INTERFACE_Device) == 0) {
    if (const auto* key_media_class = spa_dict_lookup(props, PW_KEY_MEDIA_CLASS)) {
      ...
      if (media_class == tags::pipewire::media_class::device) {
        uint64_t serial = 0U;
        if (!spa_dict_get_num(props, PW_KEY_OBJECT_SERIAL, serial)) {
          util::warning("An error occurred while converting the object serial. This device cannot be handled by Easy Effects.");
          return;
        }
        auto* proxy = static_cast<pw_proxy*>(pw_registry_bind(pm->registry, id, type, PW_VERSION_DEVICE, sizeof(proxy_data)));
        auto* const pd = static_cast<proxy_data*>(pw_proxy_get_user_data(proxy));
        pd->proxy = proxy;
        pd->pm = pm;
        pd->id = id;
...
auto PipeManager::count_node_ports(const uint& node_id) -> uint {
  return std::count_if(list_ports.begin(), list_ports.end(), [node_id](const auto& port) {
    return port.node_id == node_id;
  });
}
...
auto PipeManager::link_nodes(const uint& output_node_id,
  ...
  for (const auto& port : list_ports) {
    if (port.node_id == output_node_id && port.direction == "out") {
      ...
      if (!probe_link && (port.audio_channel != "FL" && port.audio_channel != "FR")) {
    ...
    if (port.node_id == input_node_id && port.direction == "in") {
      ...
      } else if (port.audio_channel == "PROBE_FL" || port.audio_channel == "PROBE_FR") {
        list_input_ports.push_back(port);
      }
    }
  }
  ...
  for (const auto& outp : list_output_ports) {
    for (const auto& inp : list_input_ports) {
      ...
      if (!probe_link) {
        ports_match = use_audio_channel ? (outp.audio_channel == inp.audio_channel) : (outp.port_id == inp.port_id);
      } else {
        ports_match = (outp.audio_channel == "FL" && inp.audio_channel == "PROBE_FL") ||
