// Filename: multiband_compressor_ui.cpp
...
struct _MultibandCompressorBox {
  ...
  GtkStack* stack;
  GtkCheckButton *enable_band1, *enable_band2, *enable_band3, *enable_band4, *enable_band5, *enable_band6, *enable_band7;
...
void setup(MultibandCompressorBox* self,
  ...
  gtk_label_set_text(self->plugin_credit, ui::get_plugin_credit_translated(self->data->multiband_compressor->package).c_str());
  ...
  g_settings_bind(ui::get_global_app_settings(), "show-native-plugin-ui", self->show_native_ui, "visible", G_SETTINGS_BIND_DEFAULT);
}
