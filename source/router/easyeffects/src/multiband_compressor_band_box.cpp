// Filename: multiband_compressor_band_box.cpp
...
gboolean set_boost_threshold_sensitive(MultibandCompressorBandBox* self, const guint selected_id) {
  return (selected_id == 0U || selected_id == 2U) ? 0 : 1;
}

gboolean set_boost_amount_sensitive(MultibandCompressorBandBox* self, const guint selected_id) {
  return (selected_id == 0U || selected_id == 1U) ? 0 : 1;
}

void set_label_text(GtkLabel* label, const std::string& text) {
  if (GTK_IS_WIDGET(label) && GTK_IS_LABEL(label)) {
    gtk_label_set_text(label, text.c_str());
  }
}

void set_end_label(MultibandCompressorBandBox* self, const float& value) {
  set_label_text(self->end_label, fmt::format("{0:.0f}", value));
}

void set_envelope_label(MultibandCompressorBandBox* self, const float& value) {
  set_label_text(self->envelope_label, fmt::format("{0:.0f}", util::linear_to_db(value)));
}

void set_curve_label(MultibandCompressorBandBox* self, const float& value) {
  set_label_text(self->curve_label, fmt::format("{0:.0f}", util::linear_to_db(value)));
}

void set_gain_label(MultibandCompressorBandBox* self, const float& value) {
  set_label_text(self->gain_label, fmt::format("{0:.0f}", util::linear_to_db(value)));
}
...
void multiband_compressor_band_box_init(MultibandCompressorBandBox* self) {
  ...
  prepare_spinbuttons<"ms">(self->attack_time, self->release_time, self->sidechain_reactivity, self->sidechain_lookahead);
  prepare_spinbuttons<"dB">(self->attack_threshold, self->knee, self->makeup, self->sidechain_preamp, self->boost_amount, self->boost_threshold);
