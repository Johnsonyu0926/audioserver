...
class LadspaWrapper {
 public:
  ...
  LadspaWrapper(LadspaWrapper&&) = delete;
  auto operator=(LadspaWrapper&&) -> LadspaWrapper& = delete;
  ...
  float set_control_port_value_clamp(uint index, float value);
  float set_control_port_value_clamp(const std::string& symbol, float value);

  [[nodiscard]] bool found_plugin() const { return found; }
  [[nodiscard]] bool has_instance() const { return instance != nullptr; }
  [[nodiscard]] uint get_rate() const { return rate; }
  ...
  void bind_key_double_db_exponential(GSettings* settings) {
    ...
    const auto clamped = (!lower_bound && db_v <= util::minimum_db_level) ? -std::numeric_limits<float>::infinity() : db_v;
