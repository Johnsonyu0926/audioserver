...
GtkWidget* missing_plugin_box(const std::string& base_name, const std::string& package);

void show_simple_message_dialog(GtkWidget* parent, const std::string& title, const std::string& descr);

gboolean parse_spinbutton_output(GtkSpinButton* button, const char* unit, const bool& lower_bound = true);

gint parse_spinbutton_input(GtkSpinButton* button, gdouble* new_value, const bool& lower_bound = true);

uint get_new_filter_serial();

void set_ignore_filter_idle_add(const uint& serial, const bool& state);

bool get_ignore_filter_idle_add(const uint& serial);

void save_user_locale();

std::locale get_user_locale();

std::string get_plugin_credit_translated(const std::string& plugin_package);
...
GSettings* get_global_app_settings();
