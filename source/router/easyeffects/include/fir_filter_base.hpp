...
public:
    ...
    void process(T1& data_left, T1& data_right) {
        ...
        std::copy(data_right.begin(), data_right_in.begin());
...
protected:
    ...
    [[nodiscard]] auto create_lowpass_kernel(const float& cutoff, const float& transition_band) const -> std::vector<float>;
    void setup_zita();
