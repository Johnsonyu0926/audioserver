...
void yang_create_mediadesc(YangMediaDesc* desc) {
    // Initialize YangMediaDesc structure
    if (desc == NULL) return;
    yang_memset(desc, 0, sizeof(YangMediaDesc));
}
...
int32_t yang_encode_mediadesc(YangMediaDesc* desc, YangBuffer* os) {
    ...
    for (int i = 0; i < desc->payload_types.vsize; i++) {
        yang_write_cstring(os, " ");
        yang_memset(intstr, 0, sizeof(intstr));
    ...
    for (int i = 0; i < desc->extmaps.vsize; i++) {
    ...
    for (int i = 0; i < desc->payload_types.vsize; i++) {
        if ((err = yang_encode_mediapayloadtype(&desc->payload_types.payload[i], os)) != Yang_Ok) {
            return yang_error_wrap(1, "encode media payload failed");
        }
    }

    for (int i = 0; i < desc->ssrc_infos.vsize; i++) {
        if ((err = yang_encode_ssrcinfo(&desc->ssrc_infos.payload[i], os)) != Yang_Ok) {
    ...
    for (int i = 0; i < desc->candidates.vsize; i++) {
...
int32_t yang_mediadesc_update_msid(YangMediaDesc* desc, char* id) {
    ...
    for (int i = 0; i < desc->ssrc_infos.vsize; i++) {
        ...
        yang_strncpy(info->msid, id);
        yang_memset(info->mslabel, 0, sizeof(info->mslabel));
        yang_strncpy(info->mslabel, id);
    }
    return err;
}
...
int32_t yang_sessioninfo_parse_attribute(YangSessionInfo* info, char* attribute, char* value) {
    ...
    if (yang_strcmp(attribute, "ice-ufrag") == 0) {
        ...
        yang_strncpy(info->ice_ufrag, value);
        ...
        yang_strncpy(info->ice_pwd, value);
        ...
        yang_strncpy(info->ice_options, value);
        ...
        if (p) {
            ...
            yang_strncpy(info->fingerprint, p + 1);
        }
        ...
        yang_strncpy(info->setup, value);
    }

    return err;
}

YangMediaPayloadType* yang_mediadesc_find_media_with_payload_type(YangMediaDesc* desc, int32_t payload_type) {
...
int32_t yang_mediadesc_find_encoding_name(YangMediaDesc* desc, char* encoding_name) {
    for (size_t i = 0; i < desc->payload_types.vsize; ++i) {
        if (yang_strcmp(desc->payload_types.payload[i].encoding_name, encoding_name) == 0) {
            return 1;
        }
    }
    return 0;
...
int32_t yang_mediadesc_parse_attr_extmap(YangMediaDesc* desc, char* value) {
    int32_t err = Yang_Ok;
    char* p = yang_strstr(value, " ");
    if (p == NULL) return ERROR_RTC_SDP_DECODE;
    char intstr[32];
    yang_memset(intstr, 0, sizeof(intstr));
...
int32_t yang_mediadesc_parse_attr_rtpmap(YangMediaDesc* desc, char* value) {
    ...
    YangMediaPayloadType* payload = yang_mediadesc_find_media_with_payload_type(desc, payload_type);
    if (payload == NULL) {
    ...
    if (str.vsize < 2) {
        ...
        return yang_error_wrap(ERROR_RTC_SDP_DECODE, "invalid rtpmap line=%s", value);
    }

    yang_strncpy(payload->encoding_name, str.str[0]);
    payload->clock_rate = yang_atoi(str.str[1]);

    if (str.vsize == 3) {
        yang_strncpy(payload->encoding_param, str.str[2]);
    }
    yang_destroy_strings(&str);
    return err;
}
...
int32_t yang_mediadesc_parse_attr_fmtp(YangMediaDesc* desc, char* value) {
    ...
    if (str.vsize < 1) {
        yang_destroy_strings(&str);
        return yang_error_wrap(ERROR_RTC_SDP_DECODE, "can not find payload when pase fmtp");
    }
    ...
    yang_strncpy(payload->format_specific_param, str.str[1]);
    yang_destroy_strings(&str);
    return err;
}
...
int32_t yang_mediadesc_parse_attr_msid(YangMediaDesc* desc, char* value) {
    ...
    yang_cstr_split(value, " ", &str);
    if (str.vsize > 1) {
        yang_strncpy(desc->msid, str.str[0]);
        yang_strncpy(desc->msid_tracker, str.str[1]);
    }
    yang_destroy_strings(&str);
    return err;
}
...
int32_t yang_mediadesc_parse_attr_ssrc_group(YangMediaDesc* desc, char* value) {
    ...
    if (str.vsize < 3) {
        yang_destroy_strings(&str);
        return yang_error_wrap(ERROR_RTC_SDP_DECODE, "invalid ssrc-group line=%s", value);
    }

    YangSSRCGroup group;
    yang_memset(&group, 0, sizeof(group));
    yang_strncpy(group.semantic, str.str[0]);
...
YangSSRCInfo* yang_mediadesc_fetch_or_create_ssrc_info(YangMediaDesc* desc, uint32_t ssrc) {
...
int32_t yang_mediadesc_parse_attr_ssrc(YangMediaDesc* desc, char* value) {
    ...
    YangSSRCInfo* ssrc_info = yang_mediadesc_fetch_or_create_ssrc_info(desc, ssrc);

    if (yang_strcmp(ssrc_attr, "cname") == 0) {
        // @see: https://tools.ietf.org/html/rfc5576#section-6.1
        yang_strncpy(ssrc_info->cname, ssrc_value);
        ssrc_info->ssrc = ssrc;
        ...
        if (str.vsize == 0) {
            return yang_error_wrap(ERROR_RTC_SDP_DECODE, "invalid ssrc line=%s", value);
        }

        yang_strncpy(ssrc_info->msid, str.vsize == 1 ? ssrc_value : str.str[0]);

        if (str.vsize > 1) {
            yang_strncpy(ssrc_info->msid_tracker, str.str[1]);
        }
        yang_destroy_strings(&str);
    } else if (yang_strcmp(ssrc_attr, "mslabel") == 0) {
        yang_strncpy(ssrc_info->mslabel, ssrc_value);
    } else if (yang_strcmp(ssrc_attr, "label") == 0) {
        yang_strncpy(ssrc_info->label, ssrc_value);
    }
...
int32_t yang_mediadesc_parse_attr_candidate(YangMediaDesc* desc, char* value) {
    ...
    yang_memset(&candidate, 0, sizeof(YangCandidate));

    yang_strncpy(candidate.ip, str.str[4]);
    candidate.port = atoi(str.str[5]);
    yang_strncpy(candidate.type, str.str[7]);

    yang_insert_YangCandidateVector(&desc->candidates, &candidate);
...
int32_t yang_mediadesc_parse_attribute(YangMediaDesc* desc, char* content) {
    ...
    if (p) {
        ...
        yang_strncpy(attribute, content);
    }
