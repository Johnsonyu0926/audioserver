...
void yang_sdp_genLocalSdp_payloadType(YangMediaPayloadType* videotype) {
    ...
    yang_strncpy(videotype->format_specific_param, "level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f");
...
void yang_sdp_genLocalSdp_candidate(YangRtcSession* session, YangMediaDesc* media_desc, char* localIp, int32_t localport) {
    ...
    yang_strncpy(media_desc->candidates.payload[index].ip, localIp);
    ...
    if (session->ice.session.candidateType > YangIceHost) {
        yang_strncpy(media_desc->candidates.payload[index].type, session->ice.session.candidateType == YangIceTurn ? "relay" : "srflx");
    } else {
        yang_strncpy(media_desc->candidates.payload[index].type, "host");
    }
}
...
int32_t yang_sdp_genLocalSdp2(YangRtcSession* session, int32_t localport, char* dst, YangRtcDirection role) {
    ...
    yang_strncpy(local_sdp->version, "0");
    yang_strncpy(local_sdp->username, "MetaRtc");
    yang_memset(randstr, 0, sizeof(randstr));
    yang_snprintf(randstr, 22, "%" PRId64, (int64_t)&local_sdp);
    yang_strncpy(local_sdp->session_id, randstr);
    yang_strncpy(local_sdp->session_version, "2");
    yang_strncpy(local_sdp->nettype, "IN");
    yang_strncpy(local_sdp->addrtype, "IP4");
    yang_strncpy(local_sdp->unicast_address, "0.0.0.0");
    yang_strncpy(local_sdp->session_name, "MetaRtcSession");
    yang_strncpy(local_sdp->msid_semantic, "WMS");
    ...
    yang_strncpy(audio_media_desc->type, "audio");
    audio_media_desc->port = 9;
    yang_strncpy(audio_media_desc->protos, "UDP/TLS/RTP/SAVPF");
    ...
    yang_strncpy(video_media_desc->type, "video");
    video_media_desc->port = 9;
    yang_strncpy(video_media_desc->protos, "UDP/TLS/RTP/SAVPF");
    ...
    if (session->enableDatachannel) {
        ...
        yang_strncpy(data_media_desc->type, "application");
        data_media_desc->port = 9;
        yang_strncpy(data_media_desc->protos, "UDP/DTLS/SCTP webrtc-datachannel");
    ...
    yang_strncpy(session->local_ufrag, randstr);

#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.ice_ufrag, randstr);
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.ice_ufrag, randstr);
#endif

#if Yang_Enable_Datachannel
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.ice_ufrag, randstr);
#endif
...
#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.ice_pwd, randstr);
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.ice_pwd, randstr);
#endif

#if Yang_Enable_Datachannel
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.ice_pwd, randstr);
#endif

    yang_memset(session->localIcePwd, 0, sizeof(session->localIcePwd));
    yang_strncpy(session->localIcePwd, randstr);

#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.fingerprint_algo, "sha-256");
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.fingerprint_algo, "sha-256");
#endif

#if Yang_Enable_Datachannel
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.fingerprint_algo, "sha-256");
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.ice_options, "trickle");
#endif

#if Yang_Enable_Dtls
#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.fingerprint, session->context.cer->fingerprint);
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.fingerprint, session->context.cer->fingerprint);
#endif

#if Yang_Enable_Datachannel
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.fingerprint, session->context.cer->fingerprint);
#endif

#else

#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.fingerprint, "EF:7A:50:9C:05:8C:EF:84:4D:72:B2:74:30:BA:FD:82:76:D1:C3:FE:0C:A0:10:43:B8:6C:B2:ED:B3:F7:77:8B");
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.fingerprint, "EF:7A:50:9C:05:8C:EF:84:4D:72:B2:74:30:BA:FD:82:76:D1:C3:FE:0C:A0:10:43:B8:6C:B2:ED:B3:F7:77:8B");
#endif

    if (data_media_desc) yang_strncpy(data_media_desc->session_info.fingerprint, "EF:7A:50:9C:05:8C:EF:84:4D:72:B2:74:30:BA:FD:82:76:D1:C3:FE:0C:A0:10:43:B8:6C:B2:ED:B3:F7:77:8B");
#endif

#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->session_info.setup, session->isControlled ? "passive" : "active");
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->session_info.setup, session->isControlled ? "passive" : "active");
#endif

#if Yang_Enable_Datachannel
    if (data_media_desc) yang_strncpy(data_media_desc->session_info.setup, session->isControlled ? "passive" : "active");
#endif
    ...
    yang_strncpy(video_media_desc->extmaps.payload[0].extmap, Yang_SDP_kTWCCExt);
#endif
    ...
    yang_strncpy(audiotype->encoding_name, "opus");
    audiotype->clock_rate = session->context.avinfo->audio.sample;
    yang_itoa2(session->context.avinfo->audio.channel, audiotype->encoding_param, 10);
    yang_strncpy(audiotype->format_specific_param, session->context.avinfo->audio.enableAudioFec ? "minptime=10;useinbandfec=1" : "minptime=10");
#endif
    ...
    if (mediaServer == Yang_Server_Zlm && role == YangRecvonly) {
        ...
        yang_strncpy(videotype->encoding_name, "H264");
        ...
        yang_strncpy(videotype->encoding_name, "H265");
        yang_sdp_genLocalSdp_payloadType(videotype);
    } else {
...
#if Yang_Enable_RTC_Audio
    yang_strncpy(audio_media_desc->ssrc_infos.payload[0].cname, "metaRTC");
    yang_strncpy(audio_media_desc->ssrc_infos.payload[0].mslabel, "metaRTC");
#endif

#if Yang_Enable_RTC_Video
    yang_strncpy(video_media_desc->ssrc_infos.payload[0].cname, "metaRTC");
    yang_strncpy(video_media_desc->ssrc_infos.payload[0].mslabel, "metaRTC");
#endif
    ...
    yang_strncpy(audio_media_desc->ssrc_infos.payload[0].msid, "metaRTC");
    yang_strncpy(audio_media_desc->ssrc_infos.payload[0].msid_tracker, randstr);
    yang_strncpy(audio_media_desc->ssrc_infos.payload[0].label, randstr);
#endif
    ...
    yang_memset(randstr, 0, sizeof(randstr));
    yang_sprintf(randstr, "%s-%s-%s-%s-%s", tmps[0], tmps[1], tmps[2], tmps[3], tmps[4]);
    yang_strncpy(video_media_desc->ssrc_infos.payload[0].msid, "metaRTC");
    yang_strncpy(video_media_desc->ssrc_infos.payload[0].msid_tracker, randstr);
    yang_strncpy(video_media_desc->ssrc_infos.payload[0].label, randstr);
#endif
    ...
    if (mediaServer == Yang_Server_P2p) {
        ...
        if (hasIplist) {
...
#if Yang_Enable_RTC_Audio
            for (j = 0; j < iplists.vsize; j++) {
                yang_sdp_genLocalSdp_candidate(session, audio_media_desc, iplists.payload[j], localport);
            }
#endif

#if Yang_Enable_RTC_Video
            for (j = 0; j < iplists.vsize; j++) {
                yang_sdp_genLocalSdp_candidate(session, video_media_desc, iplists.payload[j], localport);
            }
#endif
            ...
            if (data_media_desc) {
                ...
                for (j = 0; j < iplists.vsize; j++) {
                    yang_sdp_genLocalSdp_candidate(session, data_media_desc, iplists.payload[j], localport);
                }
            }
#endif
...
int32_t yang_sdp_parseRemoteSdp(YangRtcSession* session, YangSdp* sdp) {
    ...
    for (int32_t i = 0; i < sdp->media_descs.vsize; i++) {
        ...
        if (yang_strlen(session->remoteIcePwd) == 0) {
            ...
            yang_strncpy(session->remoteIcePwd, desc->session_info.ice_pwd);
        }

        if (yang_strlen(session->remote_ufrag) == 0) {
            yang_memset(session->remote_ufrag, 0, sizeof(session->remote_ufrag));
            yang_strncpy(session->remote_ufrag, desc->session_info.ice_ufrag);
        }
                ...
                if (candidate) {
                    if (candidate->port > 0) {
                        ...
                        if (replaceIp) {
                            ...
                            yang_strncpy(session->context.streamConfig->remoteIp, candidate->ip);
                            replaceIp = yangfalse;
                        }
                    }
    ...
    if (session->context.streamConfig && session->context.streamConfig->rtcCallback.setMediaConfig) {
        ...
        session->context.streamConfig->rtcCallback.setMediaConfig(session->context.streamConfig->rtcCallback.context, session->context.streamConfig->uid, remoteAudio, remoteVideo);
    }
    return Yang_Ok;
}
