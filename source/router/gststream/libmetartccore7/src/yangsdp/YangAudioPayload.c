...
void yang_init_audioPayload(YangAudioPayload* audio, int32_t channel) {
    yang_strncpy(audio->codecPayload.type, "audio", sizeof(audio->codecPayload.type) - 1);
...
int32_t yang_sdp_set_opus_param_desc(YangAudioPayload* audio, const char* fmtp) {
    int32_t err = Yang_Ok;
    YangStrings vec;
