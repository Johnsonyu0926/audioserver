...
int32_t yang_rtc_tcp_sendData_srs(YangRtcSocketSession *session, char *data, int32_t nb) {
    if (session == NULL || !session->isStart || session->notRemoteInit || data == NULL || session->fd == -1) {
        return ERROR_RTC_SOCKET;
    }
    ...
    sendBytes = nb;
    nbytes = yang_socket_send(session->fd, data, sendBytes);
...
void* yang_run_rtctcp_thread(void *obj) {
    ...
    while (sock->isLoop) {
        ...
        if ((readLen = yang_socket_recv(sock->fd, buffer, 4096, 0)) > 0) {
            ...
            if (sockerr == EINTR) continue;
            if (sockerr == EWOULDBLOCK || sockerr == EAGAIN) {
                readLen = 0;
                continue;
            }
            yang_error("%s, recv returned %d. GetSockError(): %d (%s)", __FUNCTION__, readLen, sockerr, strerror(sockerr));
            break;
        }
    }
...
void* yang_run_rtctcp_thread_srs(void *obj) {
    ...
    while (sock->isLoop) {
        if (!recvData) {
            if (msgLen == 0) msgReadLen = headerLen;
            else msgReadLen = msgLen;
        }
        if ((readLen = yang_socket_recv(sock->fd, readBuffer + msgReadBufferLen, msgReadLen, 0)) > 0) {
            ...
            if (sock->receive) sock->receive(readBuffer, msgLen, sock->user);
            msgLen = 0;
        } else if (readLen == -1) {
            int32_t sockerr = GetSockError();
            if (sockerr == EINTR) continue;
...
void* yang_run_rtctcp_thread_turn(void *obj) {
    ...
    while (sock->isLoop) {
        ...
        if ((readLen = yang_socket_recv(sock->fd, p + msgReadBufferLen, msgReadLen, 0)) > 0) {
            ...
            if (sock->receive) sock->receive(readBuffer, msgLen + headerLen - blank, sock->user);
            p = readBuffer;
            msgLen = 0;
        } else if (readLen == -1) {
            int32_t sockerr = GetSockError();
...
void yang_start_rtcudp(YangRtcSocketSession *sock) {
    ...
    if (sock->socketProtocol == Yang_Socket_Protocol_Udp) {
        ...
        if (yang_thread_create(&sock->threadId, 0, sock->headerLen == 2 ? yang_run_rtctcp_thread_srs : yang_run_rtctcp_thread_turn, sock)) {
            yang_error("YangThread::start could not start thread");
        }
    }
}
...
int32_t yang_create_rtcsocket(YangRtcSocket *psock, YangIpFamilyType familyType, YangSocketProtocol protocol, int32_t plocalPort) {
    ...
    if (sock->fd == -1) {
        return yang_error_wrap(ERROR_RTC_SOCKET, "create rtc socket fail, familyType=%s, protocol=%s",
