// Filename: yang_turn_connection.cpp
// Score: 97
...
void yang_turn_addFingerprint(YangTurnSession *session, YangBuffer *stream, yangbool addFingerprint) {
    ...
    char keystr[256] = {0};
    char key[128] = {0};
    yang_snprintf(keystr, sizeof(keystr), "%s:%s:%s", session->server->username, session->realm, session->server->password);
...
int32_t yang_turn_encode_allocate(YangTurnSession *session, uint8_t protocol, yangbool beforCredential) {
    ...
    if (!beforCredential) {
        ...
        char *nonce = NULL;
        ret = session->stun.encode_nonce((char*) session->nonce, session->nonceLen, &nonce);
    ...
    return session->sock->write(&session->sock->session, s, yang_buffer_pos(&stream));
}
...
int32_t yang_turn_encode_permission(YangTurnSession *session, YangTurnPeer *peer) {
    ...
    char *hostStr = NULL;
    ret = session->stun.encode_peer_address_ipv4(peer->address.mapAddress, peer->address.port, &hostStr);
...
int32_t yang_turn_encode_request(YangTurnSession *session) {
    ...
    session->stun.encode_request(&stream, session->sock, session->server->username, session->server->password);
    return session->sock->write(&session->sock->session, stream.data, yang_buffer_pos(&stream));
}
...
int32_t yang_turnconn_sendData(YangTurnSession *psession, int32_t uid, char *data, int32_t nb) {
    if (psession == NULL || data == NULL) return ERROR_RTC_TURN;

    YangRtcSocketSession *session = &psession->sock->session;
    int32_t nbytes = 0;
    if (session == NULL || !session->isStart || session->notRemoteInit || psession->state != YangTurnReady) {
...
void yang_turnconn_updateNonce(YangTurnSession *session) {
    ...
    if (session->response.nonceLen > 0) {
        ...
        yang_memcpy(session->nonce, session->response.nonce, session->response.nonceLen);
    }
}

int32_t yang_turnconn_handleError(YangTurnSession *session, char *data, int32_t nb) {
...
int32_t yang_turnconn_handleRecv(YangTurnSession *session, char *data, int32_t nb) {
    if (session == NULL || data == NULL) return ERROR_RTC_TURN;
    int32_t err = Yang_Ok;
    uint16_t stuntype = yang_get_be16((uint8_t*) data);
    ...
    switch (stuntype) {
        ...
        case StunAllocateSuccessResponse:
            ...
            if (session->relayPort == 0) {
                ...
                yang_trace("\nturn allocate ip=%u.%u.%u.%u,turn port=%d,lifetime=%d", ip[0], ip[1], ip[2], ip[3], session->relayPort, session->response.lifetime);
            }
            if (session->response.lifetime > 0)
                session->lifetime = session->response.lifetime;
            session->allocateOuttime = yang_get_system_time() / YANG_UTIME_SECONDS + session->lifetime;
        ...
        case StunPermissionSuccessResponse:
            for (int32_t i = 0; i < session->peers.vec.vsize; i++) {
                if (session->peers.vec.payload[i]->state == YangTurnPermission) {
                    ...
                    session->peers.vec.payload[i]->lifetimeOuttime = yang_get_system_time() / YANG_UTIME_SECONDS + Yang_Turn_Permission_Lifetime;
                }
            }
...
int32_t yang_turnconn_receive(YangTurnSession *session, char *data, int32_t nb) {
    ...
    if (yang_get_be32((uint8_t*) data + 4) == kStunMagicCookie) { // turn
        ...
        if ((stuntype >= StunBindingErrorResponse && stuntype <= StunRefreshErrorResponse) || (stuntype == StunPermissionErrorResponse || stuntype == StunChannelBindErrorResponse)) {
...
void yang_turnconn_refreshAllocate(YangTurnSession *session) {
    ...
    for (int32_t i = 0; i < session->peers.vec.vsize; i++) {
        peer = session->peers.vec.payload[i];
        if (yang_get_system_time() / YANG_UTIME_SECONDS > peer->lifetimeOuttime - 60) {
            peer->state = session->permissionCount >= 2 ? YangTurnBindChannel : YangTurnPermission;
            session->waitTime = 20;
            return;
        }
    }

    if (session->allocateOuttime && yang_get_system_time() / YANG_UTIME_SECONDS >= session->allocateOuttime - 60) {
    ...
    while (session->isLoop) {
        ...
        switch (session->state) {
            ...
            case YangTurnPermission:
                if (session->peers.vec.vsize == 0) break;
                for (int32_t i = 0; i < session->peers.vec.vsize; i++) {
                    if (session->peers.vec.payload[i]->state == YangTurnPermission) {
            ...
            case YangTurnBindChannel:
                ...
                for (int32_t i = 0; i < session->peers.vec.vsize; i++) {
                    if (session->peers.vec.payload[i]->state == YangTurnBindChannel) {
                        yang_turn_encode_bindChannel(session, session->peers.vec.payload[i]);
                    }
                }
                break;
            case YangTurnReady:
...
int32_t yang_turnconn_start(YangTurnSession *session) {
    ...
    if (yang_thread_create(&session->threadId, 0, yang_run_turnconn_handle_thread, session)) {
...
int32_t yang_turnconn_addPeer(YangTurnSession *session, int32_t uid, void *rtcSession, yang_turn_receive receive, char *remoteIp, uint16_t port) {
...
void yang_create_turnConnection(YangTurnConnection *conn, YangIceServer *server, YangRtcSocket *sock, int32_t localport) {
    ...
    session->sock->updateRemoteAddress(&session->sock->session, serverIp, session->server->serverPort);
