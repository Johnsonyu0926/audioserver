...
int32_t yang_encode_rtpHeader(YangBuffer* buf, YangRtpHeader* header) {
    ...
    // Encode the RTP fix header, 12 bytes.
...
int32_t yang_decode_rtpHeader(YangBuffer* buf, YangRtpHeader* header) {
    ...
    if (!yang_buffer_require(buf, 12)) {
        return yang_error_wrap(ERROR_RTC_RTP_MUXER, "requires %d+ bytes", 12);
    }
    ...
    if (extension) {
        if (header->extensions == NULL) {
            header->extensions = (YangRtpExtensions*)yang_calloc(1, sizeof(YangRtpExtensions));
        }
    ...
    if (padding && !header->ignore_padding && !yang_buffer_empty(buf)) {
        ...
        if (!yang_buffer_require(buf, header->padding_length)) {
            return yang_error_wrap(ERROR_RTC_RTP_MUXER, "padding requires %d bytes", header->padding_length);
        }
    }
