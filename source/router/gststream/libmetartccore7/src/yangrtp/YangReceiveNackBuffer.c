...
int32_t yang_nackbuffer_size(YangReceiveNackBuffer* nackbuf) {
    return yang_rtp_seq_distance(nackbuf->begin, nackbuf->end);
}
...
void yang_nackbuffer_clear_history(YangReceiveNackBuffer* nackbuf, uint16_t seq) {
...
void yang_nackbuffer_clear_all_history(YangReceiveNackBuffer* nackbuf) {
    for (uint16_t i = 0; i < nackbuf->capacity; i++) {
        nackbuf->queue[i] = -1;
    }
}
...
void yang_nackbuffer_notify_nack_list_full(YangReceiveNackBuffer* nackbuf) {
    yang_nackbuffer_clear_all_history(nackbuf);
    nackbuf->begin = nackbuf->end = 0;
    nackbuf->contexttialized = yangfalse;
}
