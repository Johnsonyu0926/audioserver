...
int32_t yang_create_certificate(YangCertificate *cer) {
    ...
    if (cer->dtls_cert == NULL)
        cer->dtls_cert = (mbedtls_x509_crt*) yang_calloc(sizeof(mbedtls_x509_crt), 1);
    if (cer->dtls_pkey == NULL)
        cer->dtls_pkey = (mbedtls_pk_context*) yang_calloc(sizeof(mbedtls_pk_context), 1);

    entropy = (mbedtls_entropy_context*) yang_calloc(sizeof(mbedtls_entropy_context), 1);
    ctrDrbg = (mbedtls_ctr_drbg_context*) yang_calloc(sizeof(mbedtls_ctr_drbg_context), 1);
    tmpCert = (mbedtls_x509write_cert*) yang_calloc(sizeof(mbedtls_x509write_cert), 1);
    ...
    if ((err = mbedtls_ctr_drbg_seed(ctrDrbg, mbedtls_entropy_func, entropy, NULL, 0)) != Yang_Ok) {
    ...
    // generate a key
    if (cer->ecdsa_mode) {
        if (mbedtls_pk_setup(cer->dtls_pkey, mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY)) != Yang_Ok
                || mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP256R1, mbedtls_pk_ec(*cer->dtls_pkey), mbedtls_ctr_drbg_random, ctrDrbg) != Yang_Ok) {
            yang_error("certificate create eckey fail");
            err = 1;
            goto ret;
        }
    } else {
        // RSA_F4 0x10001
        if (mbedtls_pk_setup(cer->dtls_pkey, mbedtls_pk_info_from_type(MBEDTLS_PK_RSA)) != Yang_Ok
                || mbedtls_rsa_gen_key(mbedtls_pk_rsa(*cer->dtls_pkey), mbedtls_ctr_drbg_random, ctrDrbg, 1024, 0x10001L) != Yang_Ok) {
            yang_error("certificate create pk_rsa fail");
            err = 1;
            goto ret;
        }
    }
    ...
    caLen = mbedtls_x509write_crt_der(tmpCert, certBuf, 4096, mbedtls_ctr_drbg_random, ctrDrbg);
    if (caLen < 0) {
        yang_error("crt generate fail(mbedtls_x509write_crt_der), ca length=%04x", caLen);
        goto ret;
    }

    if ((err = mbedtls_x509_crt_parse_der(cer->dtls_cert, certBuf + 4096 - caLen, caLen)) != Yang_Ok) {
        yang_error("crt generate fail(mbedtls_x509_crt_parse_der), err=%d, err=%04x", err, -err);
        goto ret;
    }
...
void yang_destroy_certificate(YangCertificate *cer) {
    if (cer == NULL) return;
    if (cer->dtls_cert) mbedtls_x509_crt_free(cer->dtls_cert);
    if (cer->dtls_pkey) mbedtls_pk_free(cer->dtls_pkey);
