...
int32_t yang_hmac_encode(const char* algo, const char* key, const int32_t key_length,
    ...
    HMAC_CTX_init(&hctx);
    if (&hctx == NULL) {
        return yang_error_wrap(ERROR_SSL, "hmac init failed");
    }
    ...
    if (HMAC_Update(&hctx, (const uint8_t*)input, input_length) < 0) {
        HMAC_CTX_cleanup(&hctx);
        return yang_error_wrap(ERROR_SSL, "hmac update failed");
    }

    if (HMAC_Final(&hctx, (uint8_t*)output, output_length) < 0) {
        HMAC_CTX_cleanup(&hctx);
        return yang_error_wrap(ERROR_SSL, "hmac final failed");
    }
...
int32_t yang_create_certificate(YangCertificate* cer) {
    ...
    X509_NAME_add_entry_by_txt(subject, "CN", MBSTRING_ASC, (const uint8_t*)"metaRTC", (int)yang_strlen("metaRTC"), -1, 0);
