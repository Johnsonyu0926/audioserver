...
int32_t yang_filter_data(YangDtlsSession *dtls, uint8_t *data, int32_t size) {
    ...
    if (dtls->state == YangDtlsStateInit && size > 14 && data[0] == 22 && data[13] == 1) {
...
#if Yang_Mbedtls_3
void yang_dtls_keyCallback(void *user, mbedtls_ssl_key_export_type type, const unsigned char *secret, size_t secret_len, const unsigned char client_random[32], const unsigned char server_random[32], mbedtls_tls_prf_types tls_prf_type) {
#else
int yang_dtls_keyCallback(void *user, const unsigned char *secret, const unsigned char *kb, size_t maclen, size_t keylen, size_t ivlen, const unsigned char client_random[32], const unsigned char server_random[32], mbedtls_tls_prf_types tls_prf_type) {
#endif
...
int32_t yang_get_srtp_key(YangDtlsSession *session, char *precv_key, int *precvkeylen, char *psend_key, int *psendkeylen) {
    ...
    if (mbedtls_ssl_tls_prf(session->tlsPrf, session->masterSecret, sizeof(session->masterSecret), "EXTRACTOR-dtls_srtp", session->randBytes, sizeof(session->randBytes), keyingMaterialBuffer, sizeof(keyingMaterialBuffer)) != Yang_Ok) {
...
void yang_stop_rtcdtls(YangDtlsSession *dtls) {
    if (!dtls) return;
    dtls->isLoop = 0;
    while (dtls->isStart) yang_usleep(1000);
}
...
void yang_mbed_initDtls(YangDtlsSession* session) {
    ...
    if (mbedtls_ssl_config_defaults(session->sslConfig, session->isControled ? MBEDTLS_SSL_IS_SERVER : MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_DATAGRAM, MBEDTLS_SSL_PRESET_DEFAULT) != Yang_Ok) {
