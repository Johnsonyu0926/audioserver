...
int32_t yang_run_rtcdtls_app(YangDtlsSession *dtls) {
    ...
    for (int32_t i = 0; arq_count < arq_max_retry && i < max_loop; i++) {
        ...
        if (dtls->state != YangDtlsStateClientHello && dtls->state != YangDtlsStateClientCertificate) {
            return err;
        }
...
SSL_CTX* yang_build_dtls_ctx(YangDtlsSession *dtls, char *role) {
    ...
    SSL_CTX_set_verify(dtls_ctx, SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE, yang_verify_callback);
