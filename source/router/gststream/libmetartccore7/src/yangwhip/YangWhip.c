...
int32_t yang_whip_getSignal(YangRtcSession* session, const char* url, char** premoteSdp, const char* localSdp) {
    int32_t err = Yang_Ok;
    ...
    if (yang_strstr(remoteSdp, "201") && yang_strstr(remoteSdp, "Created")) {
        ...
        if (p2 == nullptr) return ERROR_RTC_Whip;
...
int32_t yang_whip_connectPeer(YangRtcConnection* conn, const char* url) {
    ...
    int err = Yang_Ok;
    char* remoteSdp = nullptr;
    char* localSdp = nullptr;
