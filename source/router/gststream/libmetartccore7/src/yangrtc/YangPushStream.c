...
int32_t yang_rtcpush_cache_nack(YangRtcPushStream *pub, YangRtpPacket *pkt, char *p, int plen) {
...
int32_t yang_rtcpush_on_rtcp_rr(YangRtcContext *context, YangRtcPushStream *pub, YangRtcpCommon *rtcp) {
    int32_t err = Yang_Ok;
...
int32_t yang_rtcpush_on_rtcp_xr(YangRtcContext *context, YangRtcPushStream *pub, YangRtcpCommon *rtcp) {
    int32_t err = Yang_Ok;
    return err;
}
...
int32_t yang_rtcpush_check_bandwidth(YangRtcContext* context, YangRtcPushStream* pub) {
    return context->bandwidth.checkBandWidth(&context->bandwidth.session, &context->stats, context->streamConfig, pub->videoSsrc);
}

YangSendNackBuffer* yang_rtcpush_fetch_rtp_packet(YangRtcContext *context, YangRtcPushStream *pub, YangPublishNackBuffer *que, uint16_t seq) {
    if (context == NULL || pub == NULL)
        return NULL;
    YangSendNackBuffer *pkt = yang_pubnackbuffer_at(que, seq);
...
int32_t yang_rtcpush_on_recv_nack(YangRtcContext *context, YangRtcPushStream *pub, YangPublishNackBuffer *que, YangRtcpNack *nack) {
    if (context == NULL || pub == NULL)
        return ERROR_RTC_PUBLISH;
    int32_t err = Yang_Ok;
    ...
    for (int32_t i = 0; i < nack->vsize; ++i) {
        ...
        YangSendNackBuffer *pkt = yang_rtcpush_fetch_rtp_packet(context, pub, que, seq);
        if (pkt == NULL) {
            continue;
        }
        yang_trace("\nsendlostsn=%hu,", seq);
...
int32_t yang_rtcpush_on_rtcp_nack(YangRtcContext *context, YangRtcPushStream *pub, YangRtcpCommon *rtcp) {
...
int32_t yang_rtcpush_do_request_keyframe(YangRtcContext *context, uint32_t ssrc) {
    if (context == NULL)
        return ERROR_RTC_PUBLISH;
    int32_t err = Yang_Ok;
...
int32_t yang_rtcpush_on_rtcp_ps_feedback(YangRtcContext *context, YangRtcPushStream *pub, YangRtcpCommon *rtcp) {
    if (context == NULL || pub == NULL)
        return ERROR_RTC_PUBLISH;
    int32_t err = Yang_Ok;
    ...
    switch (fmt) {
        ...
        default: {
            return yang_error_wrap(ERROR_RTC_RTCP, "unknown payload specific feedback=%u", fmt);
        }
    }
...
int32_t yang_rtcpush_on_rtcp(YangRtcContext *context, YangRtcPushStream *pub, YangRtcpCommon *rtcp) {
    if (context == NULL || pub == NULL)
        return ERROR_RTC_PUBLISH;
...
void yang_create_rtcpush(YangRtcPush *ppush, uint32_t audioSsrc, uint32_t videoSsrc) {
    ...
    ppush->check_bandwidth = yang_rtcpush_check_bandwidth;
