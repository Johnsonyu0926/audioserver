...
void yang_create_playTrackH264(YangRtcContext *context, YangPlayTrackH264 *track) {
    ...
    yang_memset(track->cache_video_pkts, 0, YANG_RTC_RECV_BUFFER_COUNT * sizeof(YangH264PacketCache));
...
void yang_playtrackH264_clear_cached_video(YangPlayTrackH264 *track) {
    for (size_t i = 0; i < YANG_RTC_RECV_BUFFER_COUNT; i++) {
        if (track->cache_video_pkts[i].in_use) {
            yang_memset(&track->cache_video_pkts[i], 0, sizeof(YangH264PacketCache));
        }
    }
}

void yang_playtrackH264_copy(YangPlayTrackH264 *track, YangRtpPacket *src, YangH264PacketCache *pkt) {
...
int32_t yang_playtrackH264_put_frame_video(YangRtcContext *context, YangPlayTrackH264 *track, char *p, int64_t timestamp, int32_t nb) {
...
int32_t yang_playtrackH264_check_frame_complete(YangPlayTrackH264 *track, const uint16_t start, const uint16_t end) {
...
int32_t yang_playtrackH264_find_next_lost_sn(YangPlayTrackH264 *track, uint16_t current_sn, uint16_t *end_sn) {
...
int32_t yang_playtrackH264_is_keyframe(YangRtpPacket *pkt) {
    if (pkt->nalu_type == kStapA) {
        YangAvcNaluType nalu_type = (YangAvcNaluType) ((*(pkt->payload + 3)) & kNalTypeMask);
        if (nalu_type == YangAvcNaluTypeSPS || nalu_type == YangAvcNaluTypePPS) {
            return yangtrue;
        }
    } else {
        if ((pkt->nalu_type == YangAvcNaluTypeIDR) || (pkt->nalu_type == YangAvcNaluTypeSPS) || (pkt->nalu_type == YangAvcNaluTypePPS)) {
            return yangtrue;
        }
    }
    return yangfalse;
...
int32_t yang_playtrackH264_packet_video(YangRtcContext *context, YangPlayTrackH264 *track, const uint16_t start, const uint16_t end) {
    int32_t err = Yang_Ok;
    uint16_t cnt = end - start + 1;
    YangH264PacketCache *header = &track->cache_video_pkts[yang_cache_index(start)];
    ...
    if ((err = yang_playtrackH264_put_frame_video(context, track, track->video_buffer, timestamp, p - track->video_buffer)) != Yang_Ok) {
...
int32_t yang_playtrackH264_packet_video_key_frame(YangRtcContext *context, YangPlayTrackH264 *track, YangRtpPacket *pkt) {
    int32_t err = Yang_Ok;
...
int32_t yang_playtrackH264_packet_video_Raw(YangRtcContext *context, YangPlayTrackH264 *track, YangRtpPacket *pkt) {
    int err = Yang_Ok;
    char *p = track->video_buffer;
