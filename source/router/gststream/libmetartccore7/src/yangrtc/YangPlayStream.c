...
YangPlayTrackVideo* yang_rtcplay_get_video_track(YangRtcContext *context, YangRtcPlayStream *play, uint32_t ssrc) {
    if (context == NULL || play == NULL) return NULL;
...
YangPlayTrackAudio* yang_rtcplay_get_audio_track(YangRtcContext *context, YangRtcPlayStream *play, uint32_t ssrc) {
    if (context == NULL || play == NULL) return NULL;
...
void yang_rtcplay_update_send_report_time(YangRtcContext *context, YangRtcPlayStream *play, uint32_t ssrc, YangNtp *ntp, uint32_t rtp_time) {
    if (context == NULL || play == NULL) return;
#if Yang_Enable_RTC_Video
    YangPlayTrackVideo *video_track = yang_rtcplay_get_video_track(context, play, ssrc);
    if (video_track) {
...
int32_t yang_rtcplay_send_rtcp_rr(YangRtcContext *context, YangRtcPlayStream *play) {
    ...
    int32_t err = Yang_Ok;
#if Yang_Enable_RTC_Video
    if ((err = play->videoTrack->send_rtcp_rr(context, &play->videoTrack->session.track)) != Yang_Ok) {
        return yang_error_wrap(err, "track=%u", play->videoTrack->session.track.ssrc);
    }
#endif
#if Yang_Enable_RTC_Audio
    if ((err = play->audioTrack->send_rtcp_rr(context, &play->audioTrack->session)) != Yang_Ok) {
...
#endif
#if Yang_Enable_RTC_Audio
    if ((err = play->audioTrack->send_rtcp_xr_rrtr(context, &play->audioTrack->session)) != Yang_Ok) {
        return yang_error_wrap(err, "track=%u", play->audioTrack->session.ssrc);
    }
#endif
    return err;
...
int32_t yang_rtcplay_on_rtp(YangRtcContext *context, YangRtcPlayStream *play, char *data, int32_t nb_data) {
    ...
    if ((err = yang_dec_rtp(&context->srtp, plaintext, &nb_plaintext)) != Yang_Ok) {
        ...
        return yang_error_wrap(err, "marker=%u, pt=%u, seq=%u, ts=%u, ssrc=%u, pad=%u, payload=%uB", h.marker, h.payload_type, h.sequence, h.timestamp, h.ssrc, h.padding_length, nb_data - yang_buffer_pos(&b));
    }
#endif
...
int32_t yang_rtcplay_check_send_nacks(YangRtcContext *context, YangRtcPlayStream *play) {
    if (context == NULL || play == NULL) return ERROR_RTC_PLAY;
    int32_t err = Yang_Ok;
#if Yang_Enable_RTC_Video
    if ((err = play->videoTrack->check_nacks(context, &play->videoTrack->session)) != Yang_Ok) {
        return yang_error_wrap(err, "video track=%d", play->videoTrack->session.track.ssrc);
    }
#endif
#if Yang_Enable_RTC_Audio
    if ((err = play->audioTrack->check_nacks(context, &play->audioTrack->session)) != Yang_Ok) {
...
int32_t yang_rtcplay_on_rtcp_xr(YangRtcContext *context, YangRtcPlayStream *play, YangRtcpCommon *rtcp) {
    ...
    if (pt != kXR) return yang_error_wrap(ERROR_RTC_RTCP_CHECK, "invalid XR packet");

    uint16_t length = (yang_read_2bytes(&stream) + 1) * 4;
    yang_read_4bytes(&stream);

    if (length > rtcp->nb_data) {
        return yang_error_wrap(ERROR_RTC_RTCP_CHECK, "invalid XR packet, length=%u, nb_buf=%d", length, rtcp->nb_data);
    }
    ...
    while (yang_buffer_pos(&stream) + 4 < length) {
        ...
        if (yang_buffer_pos(&stream) + block_length - 4 > rtcp->nb_data) {
            return yang_error_wrap(ERROR_RTC_RTCP_CHECK, "invalid XR packet block, block_length=%u, nb_buf=%d", block_length, rtcp->nb_data);
        }
        ...
        if (bt == 5) {
            for (int32_t i = 4; i < block_length; i += 12) {
                ...
                YangNtp cur_ntp;
                yang_ntp_from_time_ms(&cur_ntp, yang_update_system_time() / 1000);
                uint32_t compact_ntp = (cur_ntp.ntp_second << 16) | (cur_ntp.ntp_fractions >> 16);

                int32_t rtt_ntp = compact_ntp - lrr - dlrr;
                int32_t rtt = ((rtt_ntp * 1000) >> 16) + ((rtt_ntp >> 16) * 1000);
                yang_trace("ssrc=%u, compact_ntp=%u, lrr=%u, dlrr=%u, rtt=%d", ssrc, compact_ntp, lrr, dlrr, rtt);
                if (context->streamConfig->direction == YangRecvonly) yang_rtcplay_update_rtt(context, play, ssrc, rtt);
            }
        } else if (bt == 4) {
...
int32_t yang_rtcplay_on_rtcp_sr(YangRtcContext *context, YangRtcPlayStream *play, YangRtcpCommon *rtcp) {
    ...
    yang_rtcplay_update_send_report_time(context, play, rtcp->ssrc, &yang_ntp, rtcp->sr->rtp_ts);
    return err;
}
