...
int32_t yang_playvideoTrack_on_rtp(YangRtcContext *context, YangPlayTrackSession *videotrack, YangRtpPacket *src) {
...
int32_t yang_playvideoTrack_check_send_nacks(YangRtcContext *context, YangPlayTrackSession *videotrack) {
    int32_t err = Yang_Ok;
    uint32_t timeout_nacks = 0;

    if ((err = yang_playtrack_do_check_send_nacks(context, &videotrack->track, &timeout_nacks)) != Yang_Ok) {
...
void yang_playvideoTrack_setRequestKeyframeState(YangPlayTrackSession* track, int32_t state) {
    if (track->h264Track) {
        ((YangPlayTrackH264*)track->h264Track)->hasRequestKeyframe = state;
    }
    if (track->h265Track) {
        ((YangPlayTrackH265*)track->h265Track)->hasRequestKeyframe = state;
    }
}
...
void yang_create_playvideoTrack(YangRtcContext *context, YangPlayTrackVideo *track) {
    ...
    if (context->videoCodec == Yang_VED_H265) {
        YangPlayTrackH265* h265Track = (YangPlayTrackH265*) yang_calloc(1, sizeof(YangPlayTrackH265));
