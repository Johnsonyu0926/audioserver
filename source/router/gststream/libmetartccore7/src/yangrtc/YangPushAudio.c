...
int32_t yang_push_audio_encodeAudio(YangRtcSession *session, YangPushAudioRtp *rtp, YangRtpPacket *pkt) {
    int err = 0;
    ...
    if ((err = yang_encode_rtpHeader(&rtp->buf, &pkt->header)) != Yang_Ok) {
        return yang_error_wrap(err, "audio rtp header(%d) encode packet fail", pkt->payload_type);
    }
    err = yang_encode_h264_raw(&rtp->buf, &rtp->audioRawData);
    if (err != Yang_Ok) {
        return yang_error_wrap(err, "audio rtp payload(%d) encode packet fail", pkt->payload_type);
    }
...
int32_t yang_push_audio_rtp(void *psession, YangPushAudioRtp *rtp, YangFrame *audioFrame) {
    int err = 0;
    ...
    yang_memcpy(rtp->audioRawData.payload, audioFrame->payload, rtp->audioRawData.nb);
    if ((err = yang_push_audio_encodeAudio(session, rtp, &rtp->audioRawPacket)) != Yang_Ok) {
