...
void yang_create_playTrackH265(YangRtcContext *context, YangPlayTrackH265 *track) {
    ...
    yang_memset(track->cache_video_pkts, 0, YANG_RTC_RECV_BUFFER_COUNT * sizeof(YangH265PacketCache));
...
void yang_playtrackH265_clear_cached_video(YangPlayTrackH265 *track) {
    for (size_t i = 0; i < YANG_RTC_RECV_BUFFER_COUNT; i++) {
        if (track->cache_video_pkts[i].in_use) {
            yang_memset(&track->cache_video_pkts[i], 0, sizeof(YangH265PacketCache));
        }
    }
}

int32_t yang_playtrackH265_put_frame_video(YangRtcContext *context, YangPlayTrackH265 *track, char *p, int64_t timestamp, int32_t nb) {
...
int32_t yang_playtrackH265_check_frame_complete(YangPlayTrackH265 *track, const uint16_t start, const uint16_t end) {
...
int32_t yang_playtrackH265_find_next_lost_sn(YangPlayTrackH265 *track, uint16_t current_sn, uint16_t *end_sn) {
    uint32_t last_ts = track->cache_video_pkts[yang_cache_index(track->header_sn)].ts;
    ...
    yang_error("the cache is mess. the packet count of video frame is more than %u", YANG_RTC_RECV_BUFFER_COUNT);
    return -2;
}

int32_t yang_playtrackH265_is_keyframe2(YangH265PacketCache *pkt) {
    if (pkt->nalu_type == H265_kStapA) {
        YangHevcNaluType nalu_type = (YangHevcNaluType) (YANG_HEVC_NALU_TYPE(*(pkt->payload + 4)));
        if (nalu_type == YANG_NAL_UNIT_VPS || nalu_type == YANG_NAL_UNIT_SPS || nalu_type == YANG_NAL_UNIT_VPS) {
            return yangtrue;
        }
    } else {
        if ((pkt->nalu_type == YANG_NAL_UNIT_VPS) || (pkt->nalu_type == YANG_NAL_UNIT_SPS) || (pkt->nalu_type == YANG_NAL_UNIT_PPS)) {
            return yangtrue;
        }
    }
    return yangfalse;
...
void yang_playtrackH265_copy(YangPlayTrackH265 *track, YangRtpPacket *src, YangH265PacketCache *pkt) {
...
int32_t yang_playtrackH265_packet_video(YangRtcContext *context, YangPlayTrackH265 *track, const uint16_t start, const uint16_t end) {
    int32_t err = Yang_Ok;
    uint16_t cnt = end - start + 1;
    YangH265PacketCache *header = &track->cache_video_pkts[yang_cache_index(start)];
    ...
    if ((err = yang_playtrackH265_put_frame_video(context, track, track->video_buffer, timestamp, p - track->video_buffer)) != Yang_Ok) {
        yang_warn("fail to pack video frame");
    }

    track->header_sn = end + 1;
    uint16_t tail_sn = 0;
    int32_t sn = yang_playtrackH265_find_next_lost_sn(track, track->header_sn, &tail_sn);
    if (-1 == sn) {
        if (yang_playtrackH265_check_frame_complete(track, track->header_sn, tail_sn)) {
            err = yang_playtrackH265_packet_video(context, track, track->header_sn, tail_sn);
        }
    } else if (-2 == sn) {
...
int32_t yang_playtrackH265_packet_video_key_frame(YangRtcContext *context, YangPlayTrackH265 *track, YangRtpPacket *pkt) {
    int32_t err = Yang_Ok;
    ...
    if (-1 == sn) {
        if (yang_playtrackH265_check_frame_complete(track, track->header_sn, tail_sn)) {
            if ((err = yang_playtrackH265_packet_video(context, track, track->header_sn, tail_sn)) != Yang_Ok) {
...
int32_t yang_playtrackH265_packet_video_Raw(YangRtcContext *context, YangPlayTrackH265 *track, YangRtpPacket *pkt) {
    int err = Yang_Ok;
    char *p = track->video_buffer;
...
int32_t yang_playtrackH265_on_rtp(YangRtcContext *context, YangPlayTrackH265 *track, YangRtpPacket *src) {
    ...
    if (track->lost_sn == src->header.sequence) {
        uint16_t tail_sn = 0;
        int32_t sn = yang_playtrackH265_find_next_lost_sn(track, track->lost_sn, &tail_sn);
        if (-1 == sn) {
            if (yang_playtrackH265_check_frame_complete(track, track->header_sn, tail_sn)) {
                if ((err = yang_playtrackH265_packet_video(context, track, track->header_sn, tail_sn)) != Yang_Ok) {
