...
void yang_do_request_keyframe(YangRtcContext *context, uint32_t ssrc) {
    if (context && context->streamConfig && context->streamConfig->rtcCallback.sendRequest) {
        context->streamConfig->rtcCallback.sendRequest(context->streamConfig->rtcCallback.context, context->streamConfig->uid, ssrc, Yang_Req_Sendkeyframe);
    }
}
...
int32_t yang_send_rtcp_rr(YangRtcContext *context, yangbool isAudio, uint32_t ssrc, YangReceiveNackBuffer *rtp_queue, const uint64_t last_send_systime, YangNtp *last_send_ntp) {
    uint8_t fraction_lost = 0;
    uint32_t cumulative_number_of_packets_lost = 0 & 0x7FFFFF;
    uint32_t extended_highest_sequence = yang_nackbuffer_get_extended_highest_sequence(rtp_queue);
    YangBuffer stream;
    char buf[kRtpPacketSize];
    yang_init_buffer(&stream, buf, sizeof(buf));
    yang_write_1bytes(&stream, 0x81);
...
int32_t yang_check_send_nacks(YangRtcContext *context, YangRtpRecvNack *nack, uint32_t ssrc, uint32_t *sent_nacks, uint32_t *timeout_nacks) {
    int32_t err = Yang_Ok;
