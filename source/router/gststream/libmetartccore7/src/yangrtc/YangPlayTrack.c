...
void yang_create_playTrack(YangRtcContext *context, YangPlayTrack *track, yangbool is_audio) {
    ...
    if (is_audio) {
        ...
        if (context->videoTracks.vsize > 0) 
            track->ssrc = context->videoTracks.payload[0].ssrc;
...
void yang_playtrack_update_send_report_time(YangPlayTrack *track, YangNtp *ntp, uint32_t rtp_time) {
...
int32_t yang_playtrack_send_rtcp_rr(YangRtcContext *context, yangbool isAudio, YangPlayTrack *track) {
    ...
    if ((err = yang_send_rtcp_rr(context, isAudio, ssrc, track->rtp_queue, last_time, &track->last_sender_report_ntp)) != Yang_Ok) {
