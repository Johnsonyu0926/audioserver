...
void yang_cstr_replace(char* str, char* dst, char* match, char* rep) {
    if (str == NULL || dst == NULL || match == NULL || rep == NULL) return;
    char* p = yang_strstr(str, match);
    if (!p) {
    ...
    int32_t origlen = yang_strlen(match);
    ...
    while (p) {
        char* p1 = p;
        p = yang_strstr(p1 + origlen, match);
        if (p) {
