...
void yang_audiores_init(YangAudioResampleContext* context, int32_t insample, int32_t inchannel, int32_t outsample, int32_t outchannel, int32_t ms) {
    if (context == NULL) return;
...
int32_t StereoToMono(const int16_t *src_audio, int16_t *dst_audio, int32_t samples_per_channel) {
    for (int i = 0; i < samples_per_channel; i++) {
        dst_audio[i] = ((int32_t)(src_audio[2 * i]) + src_audio[2 * i + 1]) >> 1;
    }
    return Yang_Ok;
}
