...
void yang_rtcaec_init(void* context, int32_t sampleRate, int32_t pchannel, int32_t pframeSize, int32_t echopath) {
    ...
    int32_t frame_size = aec->frameSize;
    int32_t filter_length = aec->frameSize * aec->echoPath;
...
void yang_rtcaec_echo_cancellation(void* context, const short *rec, const short *play, short *out) {
    if (context == NULL) return;
    YangRtcAecContext* aec = (YangRtcAecContext*)context;
