...
int yang_h264_decode_spspps(uint8_t *buf, uint32_t nLen, int32_t *p_width, int32_t *p_height, int32_t *p_fps) {
    ...
    *p_fps = 0;
    ...
    if (sps->nal_unit_type == 7) {
        ...
        sps->constraint_set0_flag = yang_sps_u(1, buf, startBitp);
        sps->constraint_set1_flag = yang_sps_u(1, buf, startBitp);
        sps->constraint_set2_flag = yang_sps_u(1, buf, startBitp);
        sps->constraint_set3_flag = yang_sps_u(1, buf, startBitp);
        ...
        if (sps->profile_idc == 100 || sps->profile_idc == 110 ||
            ...
            if (sps->chroma_format_idc == 3)
                sps->residual_colour_transform_flag = yang_sps_u(1, buf, startBitp);
