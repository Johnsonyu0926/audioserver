// Filename: YangZlmConnection.cpp
// Score: 95
...
void yang_destroy_zlmresponse(ZlmSdpResponseType* zlm) {
    if (zlm == nullptr) return;
    yang_free(zlm->sdp);
    yang_free(zlm->id);
}

int32_t yang_zlm_query(YangRtcSession* session, ZlmSdpResponseType* zlm, int32_t isplay, const char* ip, int32_t port, const char* purl, const char* psdp) {
    int32_t err = Yang_Ok;
    ...
    if (p == nullptr) return ERROR_STRING;
    ...
    if (reader.isString(sessionid) && (sessionid->valuestring != nullptr)) {
    ...
    if (reader.isString(jsdp) && (jsdp->valuestring != nullptr)) {
...
int32_t yang_zlm_doHandleSignal(YangRtcSession* session, ZlmSdpResponseType* zlm, const char* sdp, int32_t localport, YangRtcDirection role) {
    int32_t err = Yang_Ok;
    char apiurl[256];
    ...
    err = yang_zlm_query(session, zlm, role == YangRecvonly ? 1 : 0, session->context.streamConfig->remoteIp, session->context.streamConfig->remotePort, apiurl, sdp);
