...
typedef enum {
  ...
  ENCODER_OPTION_IDR_INTERVAL,               ///< IDR period,0/-1 means no Intra period (only the first frame); larger than 0 means the desired IDR period, must be multiple of (2^temporal_layer)
  ...
  ENCODER_OPTION_NUMBER_REF,                 ///< the number of reference frames
  ...
  ENCODER_OPTION_IS_LOSSLESS_LINK,           ///< advanced algorithmic settings
...
typedef enum {
  ...
  DECODER_OPTION_LTR_MARKING_FLAG,      ///< feedback whether current frame marks a LTR
  DECODER_OPTION_LTR_MARKED_FRAME_NUM,  ///< feedback frame num marked by current Frame
  DECODER_OPTION_ERROR_CON_IDC,         ///< indicate decoder error concealment method
  DECODER_OPTION_TRACE_LEVEL,
  DECODER_OPTION_TRACE_CALLBACK,        ///< a void (*)(void* context, int level, const char* message) function which receives log messages
  DECODER_OPTION_TRACE_CALLBACK_CONTEXT,///< context info of trace callback
  ...
  DECODER_OPTION_PROFILE,               ///< get current AU profile info, only used in GetOption
  DECODER_OPTION_LEVEL,                 ///< get current AU level info, only used in GetOption
  DECODER_OPTION_STATISTICS_LOG_INTERVAL,///< set log output interval
  DECODER_OPTION_IS_REF_PIC,            ///< feedback whether current frame is ref pic or not
  DECODER_OPTION_NUM_OF_FRAMES_REMAINING_IN_BUFFER,  ///< number of frames remaining in decoder buffer when pictures are required to be reordered into display-order
  DECODER_OPTION_NUM_OF_THREADS,        ///< number of decoding threads. The maximum thread count is equal or less than the lesser of (cpu core counts and 16).
} DECODER_OPTION;
...
/**
* @brief Feedback whether or not have VCL NAL in current AU
*/
typedef enum {
...
typedef struct {
  ...
  int    iLTRRefNum;               ///< TODO: not supported to set it arbitrarily yet
} SLTRConfig;
...
typedef enum {
  ...
  RC_TIMESTAMP_MODE = 3,   ///< rate control based on timestamp
...
enum {
  ...
  WELS_LOG_RESV        = 1 << 5,        ///< reserved log level
  WELS_LOG_LEVEL_COUNT = 6,
  WELS_LOG_DEFAULT     = WELS_LOG_WARNING   ///< default log level in Wels codec
};
...
/**
* @brief Enumerate the complexity mode
*/
typedef enum {
  LOW_COMPLEXITY = 0,              ///< the lowest complexity, the fastest speed,
  MEDIUM_COMPLEXITY,               ///< medium complexity, medium speed, medium quality
  HIGH_COMPLEXITY                  ///< high complexity, lowest speed, high quality
} ECOMPLEXITY_MODE;

/**
 * @brief Enumerate for the strategy of SPS/PPS strategy
 */
typedef enum {
...
typedef struct TagEncParamBase {
  ...
  int       iPicWidth;             ///< width of picture in luminance samples (the maximum of all layers if multiple spatial layers present)
  int       iPicHeight;            ///< height of picture in luminance samples (the maximum of all layers if multiple spatial layers present)
...
/**
* @brief SVC Encoding Parameters extension
*/
typedef struct TagEncParamExt {
  ...
  int              iNumRefFrame;   ///< number of reference frames used
  EParameterSetStrategy eSpsPpsIdStrategy; ///< different strategy in adjusting ID in SPS/PPS: 0- constant ID, 1-additional ID, 6-mapping and additional
  ...
  int              iMinQp;               ///< the minimum QP encoder supports
  unsigned int     uiMaxNalSize;         ///< the maximum NAL size. This value should not be 0 for dynamic slice mode

  /* LTR settings */
  bool             bEnableLongTermReference; ///< 1: on, 0: off
  int              iLTRRefNum;               ///< the number of LTR (long term reference), TODO: not supported to set it arbitrarily yet
  unsigned int     iLtrMarkPeriod;           ///< the LTR marked period that is used in feedback.

  /* Multi-thread settings */
  unsigned short   iMultipleThreadIdc;       ///< 0: auto (dynamic implementation internal encoder); 1: multiple threads implementation disabled; larger than 1: count number of threads
  ...
  bool             bEnableSceneChangeDetect; ///< scene change detection control
...
/**
* @brief Structure for video property
*/
typedef struct {
...
/**
* @brief Structure for layer bitstream info
*/
typedef struct {
...
/**
* @brief Structure for bitrate info
*/
...
/**
* @brief Structure for delivery status
*/
...
typedef struct TagVideoEncoderStatistics {
  ...
  float fAverageFrameSpeedInMs;                ///< average encoding time
  ...
  float fLatestFrameRate;                      ///< the frame rate in, in the last second, supposed that the input timestamp is in unit of ms
  unsigned int uiBitRate;                      ///< send rate in bits per second, calculated within the set time-window
...
typedef struct TagVideoDecoderStatistics {
  ...
  float fAverageFrameSpeedInMs;                ///< average decoding time
  float fActualAverageFrameSpeedInMs;          ///< actual average decoding time, including freezing pictures
  unsigned int uiDecodedFrameCount;            ///< number of frames
  unsigned int uiResolutionChangeTimes;        ///< uiResolutionChangeTimes
  unsigned int uiIDRCorrectNum;                ///< number of correct IDR received
  // EC on related
  unsigned int uiAvgEcRatio;                   ///< when EC is on, the average ratio of total EC areas, can be an indicator of reconstruction quality
  unsigned int uiAvgEcPropRatio;               ///< when EC is on, the rough average ratio of propagate EC areas, can be an indicator of reconstruction quality
  unsigned int uiEcIDRNum;                     ///< number of actual unintegrity IDR or not received but eced
  unsigned int uiEcFrameNum;                   ///< number of EC frames
  ...
  int iSpsNoExistNalNum;                       ///< number of SPS no exist NAL
  int iSubSpsNoExistNalNum;                    ///< number of SubSPS no exist NAL
  int iPpsNoExistNalNum;                       ///< number of PPS no exist NAL
