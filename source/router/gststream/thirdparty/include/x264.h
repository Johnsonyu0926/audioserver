...
typedef struct x264_nal_t
{
    ...
    int i_first_mb; /* If this NAL is a slice, the index of the first MB in the slice. */
    int i_last_mb;  /* If this NAL is a slice, the index of the last MB in the slice. */
    int i_payload;  /* Size of payload (including any padding) in bytes. */
    uint8_t *p_payload; /* If param->b_annexb is set, Annex-B bytestream with startcode. Otherwise, startcode is replaced with a 4-byte size. */
...
#define X264_CPU_SLOW_SHUFFLE       (1<<21) /* The Conroe has a slow shuffle unit (relative to overall SSE performance) */
#define X264_CPU_STACK_MOD4         (1<<22) /* if stack is only mod4 and not mod16 */
#define X264_CPU_SLOW_ATOM          (1<<23) /* The Atom is terrible: slow SSE unaligned loads, slow SIMD multiplies, slow SIMD variable shifts, slow pshufb, cacheline split penalties -- gather everything here that isn't shared by other CPUs to avoid making half a dozen new SLOW flags. */
#define X264_CPU_SLOW_PSHUFB        (1<<24) /* such as on the Intel Atom */
#define X264_CPU_SLOW_PALIGNR       (1<<25) /* such as on the AMD Bobcat */
...
typedef struct x264_sei_t
{
    ...
    void (*sei_free)( void* ); /* In: optional callback to free each payload AND x264_sei_payload_t when used. */
} x264_sei_t;
...
typedef struct x264_image_properties_t
{
    ...
    /* Out: SSIM of the frame luma (if x264_param_t.b_ssim is set) */
    double f_ssim;
