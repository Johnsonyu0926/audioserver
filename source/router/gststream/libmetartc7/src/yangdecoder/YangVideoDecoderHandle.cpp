// Filename: YangVideoDecoderHandle.cpp
// Score: 96
...
void YangVideoDecoderHandle::addVideoStream(uint8_t *ps, int32_t puid, int32_t pisAdd) {
    if (pisAdd) {
...
void YangVideoDecoderHandle::onAudioData(YangFrame* pframe) {
    // No implementation needed for audio data in this context
}
...
void YangVideoDecoderHandle::startLoop() {
    ...
    uint8_t *temp = srcVideo + 4;

    YangYuvType yuvType = YangI420;
