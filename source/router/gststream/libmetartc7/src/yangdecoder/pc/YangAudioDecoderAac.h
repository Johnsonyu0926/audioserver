// Filename: YangAudioDecoderAac.h
// Score: 95
...
private:
    ...
    uint8_t (*yang_NeAACDecSetConfiguration)(NeAACDecHandle hDecoder, NeAACDecConfigurationPtr config);
    long (*yang_NeAACDecInit)(NeAACDecHandle hDecoder, uint8_t *buffer, unsigned long buffer_size, unsigned long *samplerate, uint8_t *channels);
    void* (*yang_NeAACDecDecode)(NeAACDecHandle hDecoder, NeAACDecFrameInfo *hInfo, uint8_t *buffer, unsigned long buffer_size);
    void (*yang_NeAACDecClose)(NeAACDecHandle hDecoder);
};

#endif // YangAudioDecoderAac_H
