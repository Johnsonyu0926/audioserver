// Filename: YangAudioDecoderAac.cpp
// Score: 95
...
void YangAudioDecoderAac::loadLib() {
    ...
    yang_NeAACDecSetConfiguration = (uint8_t (*)(NeAACDecHandle hDecoder, NeAACDecConfigurationPtr config))m_lib.loadFunction("NeAACDecSetConfiguration");
    yang_NeAACDecInit = (long (*)(NeAACDecHandle hDecoder, uint8_t *buffer, unsigned long buffer_size, unsigned long *samplerate, uint8_t *channels))m_lib.loadFunction("NeAACDecInit");
    yang_NeAACDecDecode = (void* (*)(NeAACDecHandle hDecoder, NeAACDecFrameInfo *hInfo, uint8_t *buffer, unsigned long buffer_size))m_lib.loadFunction("NeAACDecDecode");
