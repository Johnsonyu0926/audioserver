// Filename: YangVideoDecoderFfmpeg.cpp
// Score: 95
...
void YangVideoDecoderFfmpeg::loadLib() {
    ...
    yang_av_hwframe_get_buffer = (int32_t (*)(AVBufferRef *hwframe_ctx, AVFrame *frame, int32_t flags))m_lib.loadFunction("av_hwframe_get_buffer");
    yang_avcodec_find_decoder = (AVCodec* (*)(enum AVCodecID id))m_lib.loadFunction("avcodec_find_decoder");
    yang_avcodec_find_decoder_by_name = (AVCodec *(*)(const char *name))m_lib.loadFunction("avcodec_find_decoder_by_name");
    yang_avcodec_alloc_context3 = (AVCodecContext* (*)(const AVCodec *codec))m_lib.loadFunction("avcodec_alloc_context3");
    yang_avcodec_send_packet = (int32_t (*)(AVCodecContext *avctx, const AVPacket *avpkt))m_lib.loadFunction("avcodec_send_packet");
    yang_avcodec_receive_frame = (int32_t (*)(AVCodecContext *avctx, AVFrame *frame))m_lib.loadFunction("avcodec_receive_frame");
    yang_avcodec_flush_buffers = (void (*)(AVCodecContext *avctx))m_lib.loadFunction("avcodec_flush_buffers");
...
enum AVPixelFormat get_hw_format1(AVCodecContext *ctx, const enum AVPixelFormat *pix_fmts) {
...
int32_t YangVideoDecoderFfmpeg::set_hwframe_ctx(AVPixelFormat ctxformat, AVPixelFormat swformat, YangVideoInfo *yvp, AVCodecContext *ctx, AVBufferRef *hw_device_ctx, int32_t pwid, int32_t phei) {
...
void YangVideoDecoderFfmpeg::parseRtmpHeader(uint8_t *p, int32_t pLen, int32_t *pwid, int32_t *phei, int32_t *pfps) {
    uint8_t headers[128];
    ...
    if (usingHw) {
        ...
        if ((ret = set_hwframe_ctx(ctxformat, swformat, m_context, m_codecCtx, hw_device_ctx, m_width, m_height)) < 0) {
    ...
    if (usingHw) {
        yang_av_image_fill_arrays(m_frame->data, m_frame->linesize, buffer, fmt, m_width, m_height, 1);
...
void YangVideoDecoderFfmpeg::init() {
    ...
    if (usingHw) {
        if (m_encDecType == Yang_VED_H264) {
            if (m_context->videoDecHwType == YangV_Hw_Intel) m_codec = yang_avcodec_find_decoder_by_name("h264_vaapi");
            if (m_context->videoDecHwType == YangV_Hw_Nvdia) m_codec = yang_avcodec_find_decoder_by_name("h264_nvdec");
