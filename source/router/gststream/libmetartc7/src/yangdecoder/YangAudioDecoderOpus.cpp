// Filename: YangAudioDecoderOpus.cpp
// Score: 95
...
void YangAudioDecoderOpus::loadLib() {
    ...
    yang_opus_decode = (int32_t (*)(OpusDecoder *st, const uint8_t *data, opus_int32 len, opus_int16 *pcm, int32_t frame_size, int32_t decode_fec))m_lib.loadFunction("opus_decode");
...
int32_t YangAudioDecoderOpus::yang_opus_insertData(int32_t pret, YangFrame *pframe, YangDecoderCallback *pcallback) {
    ...
    if (m_fec) {
        if (m_prePts) {
            if (pframe->pts - m_prePts > m_frameTime) {
                ret = yang_opus_decode(m_decoder, pframe->payload, pframe->nb, m_out, MAX_FRAME_SIZE, 1);
