// Filename: YangAudioDecoderHandle.cpp
// Score: 97
...
void YangAudioDecoderHandle::setMediaConfig(int32_t puid, YangAudioParam *para, YangVideoParam *video) {
    if (para == NULL) return;
    m_param->encode = para->encode;
    m_param->sample = para->sample;
    m_param->channel = para->channel;
...
void YangAudioDecoderHandle::onVideoData(YangFrame *pframe) {
    // No implementation needed for video data
}
