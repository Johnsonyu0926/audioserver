// Filename: YangDecoderMediacodec.cpp
// Score: 92
...
void YangDecoderMediacodec::parseRtmpHeader(uint8_t *p, int32_t pLen, int32_t *pwid, int32_t *phei, int32_t *pfps) {
    uint8_t headers[128];
    ...
    yang_trace("output color format=%d", localColorFMT);
...
int32_t YangDecoderMediacodec::decode(YangFrame* videoFrame, YangYuvType yuvtype, YangDecoderCallback* pcallback) {
    ...
    if (outbufidx >= 0) {
            ...
            // 解码输出的格式发生变化
        {
            ...
            tryagainCount = 0;
            yang_trace("change>>>output color format=%d, width=%d, height=%d, stride=%d", localColorFMT, m_width, m_height, stride);
            break;
        }
