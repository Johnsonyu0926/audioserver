// Filename: YangAudioPlayLinux.cpp
// Score: 94
...
int YangAudioPlayLinux::init() {
    ...
    val = m_sample;
    if ((err = snd_pcm_hw_params_set_rate_near(m_handle, hw_params, &val, 0)) < 0) {
    ...
    if ((err = snd_pcm_sw_params_set_avail_min(m_handle, sw_params, m_frames)) < 0) {
        yang_error("cannot set minimum available count (%s)", snd_strerror(err));
        _exit(1);
    }
    if ((err = snd_pcm_sw_params_set_start_threshold(m_handle, sw_params, m_frames)) < 0) {
