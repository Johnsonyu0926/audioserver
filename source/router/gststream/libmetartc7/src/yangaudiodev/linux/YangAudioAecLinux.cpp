// Filename: YangAudioAecLinux.cpp
// Score: 96
...
int32_t YangAudioAecLinux::alsa_device_open(char *device_name, uint32_t rate, int32_t channels, int32_t period) {
    int32_t dir;
    int32_t err;
    ...
    m_dev = (YangAlsaDevice*) malloc(sizeof(YangAlsaDevice));
    if (!m_dev)
    ...
    strncpy(m_dev->device_name, device_name, strlen(device_name) + 1);
    m_dev->channels = channels;
    m_dev->period = period;
    err = snd_output_stdio_attach(&jcd_out, stdout, 0);

    if ((err = snd_pcm_open(&m_dev->capture_handle, m_dev->device_name, SND_PCM_STREAM_CAPTURE, 0)) < 0) {
    ...
    if (catpureDeviceState) {
        if ((err = snd_pcm_hw_params_malloc(&hw_params)) < 0) {
            yang_error("cannot allocate hardware parameter structure (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_any(m_dev->capture_handle, hw_params)) < 0) {
            yang_error("cannot initialize hardware parameter structure (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_access(m_dev->capture_handle, hw_params, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
            yang_error("cannot set access type (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_format(m_dev->capture_handle, hw_params, SND_PCM_FORMAT_S16_LE)) < 0) {
            yang_error("cannot set sample format (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_rate_near(m_dev->capture_handle, hw_params, &rate, 0)) < 0) {
            yang_error("cannot set sample rate (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_channels(m_dev->capture_handle, hw_params, channels)) < 0) {
        ...
        period_size = period;
        dir = 0;
        if ((err = snd_pcm_hw_params_set_period_size_near(m_dev->capture_handle, hw_params, &period_size, &dir)) < 0) {
        ...
        dir = 0;
        if ((err = snd_pcm_hw_params_set_buffer_size_near(m_dev->capture_handle, hw_params, &buffer_size)) < 0) {
        ...
        if ((err = snd_pcm_sw_params_malloc(&sw_params)) < 0) {
            yang_error("cannot allocate software parameters structure (%s)", snd_strerror(err));
            _exit(1);
        }
        if ((err = snd_pcm_sw_params_current(m_dev->capture_handle, sw_params)) < 0) {
            yang_error("cannot initialize software parameters structure (%s)", snd_strerror(err));
            _exit(1);
        }
        if ((err = snd_pcm_sw_params_set_avail_min(m_dev->capture_handle, sw_params, period)) < 0) {
            yang_error("cannot set minimum available count (%s)", snd_strerror(err));
            _exit(1);
        }
    ...
    if ((err = snd_pcm_open(&m_dev->playback_handle, m_dev->device_name, SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
        yang_error("cannot open audio device %s (%s)", m_dev->device_name, snd_strerror(err));
        playDeviceState = 0;
    }
    if (playDeviceState) {
        ...
        if ((err = snd_pcm_hw_params_set_access(m_dev->playback_handle, hw_params, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
            yang_error("cannot set access type (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_format(m_dev->playback_handle, hw_params, SND_PCM_FORMAT_S16_LE)) < 0) {
            yang_error("cannot set sample format (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_rate_near(m_dev->playback_handle, hw_params, &rate, 0)) < 0) {
            yang_error("cannot set sample rate (%s)", snd_strerror(err));
            _exit(1);
        }

        if ((err = snd_pcm_hw_params_set_channels(m_dev->playback_handle, hw_params, channels)) < 0) {
            yang_error("cannot set channel count (%s)", snd_strerror(err));
            _exit(1);
        }
        ...
        period_size = period;
        dir = 0;
        if ((err = snd_pcm_hw_params_set_period_size_near(m_dev->playback_handle, hw_params, &period_size, &dir)) < 0) {
            yang_error("cannot set period size (%s)", snd_strerror(err));
            _exit(1);
        }
        ...
        buffer_size = period_size * 2;
        dir = 0;
        if ((err = snd_pcm_hw_params_set_buffer_size_near(m_dev->playback_handle, hw_params, &buffer_size)) < 0) {
            yang_error("cannot set buffer time (%s)", snd_strerror(err));
            _exit(1);
        }
        ...
        if ((err = snd_pcm_sw_params_current(m_dev->playback_handle, sw_params)) < 0) {
            yang_error("cannot initialize software parameters structure (%s)", snd_strerror(err));
            _exit(1);
        }
        if ((err = snd_pcm_sw_params_set_avail_min(m_dev->playback_handle, sw_params, period)) < 0) {
            yang_error("cannot set minimum available count (%s)", snd_strerror(err));
            _exit(1);
        }
        if ((err = snd_pcm_sw_params_set_start_threshold(m_dev->playback_handle, sw_params, period)) < 0) {
        ...
        if ((err = snd_pcm_prepare(m_dev->capture_handle)) < 0) {
            yang_error("cannot prepare audio interface for use (%s)", snd_strerror(err));
            _exit(1);
        }
    ...
    if (catpureDeviceState) {
        ...
        if (snd_pcm_poll_descriptors(m_dev->capture_handle, m_dev->read_fd, m_dev->readN) != m_dev->readN) {
    ...
    if (playDeviceState) {
        ...
        if (snd_pcm_poll_descriptors(m_dev->playback_handle, m_dev->write_fd, m_dev->writeN) != m_dev->writeN) {
            yang_error("cannot obtain playback file descriptors (%s)", snd_strerror(err));
            _exit(1);
        }
    }
...
int32_t YangAudioAecLinux::alsa_device_read(short *pcm, int32_t len) {
    if ((m_ret = snd_pcm_readi(m_dev->capture_handle, pcm, len)) != len) {
        if (m_ret < 0) {
            if (m_ret == -EPIPE) {
                yang_error("An overrun has occurred, resetting capture");
            } else {
                yang_error("read from audio interface failed (%s)", snd_strerror(m_ret));
            }
            if ((m_ret = snd_pcm_prepare(m_dev->capture_handle)) < 0) {
                yang_error("cannot prepare audio interface for use (%s)", snd_strerror(m_ret));
            }
            if ((m_ret = snd_pcm_start(m_dev->capture_handle)) < 0) {
                yang_error("cannot prepare audio interface for use (%s)", snd_strerror(m_ret));
            }
        } else {
            yang_error("Couldn't read as many samples as I wanted (%d instead of %d)", m_ret, len);
        }
        return 1;
    }
    return Yang_Ok;
...
int32_t YangAudioAecLinux::alsa_device_write(const short *pcm, int32_t len) {
    if ((m_ret = snd_pcm_writei(m_dev->playback_handle, pcm, len)) != len) {
        if (m_ret < 0) {
            if (m_ret == -EPIPE) {
                yang_error("An underrun has occurred, resetting playback, len=%d", len);
            } else {
                yang_error("write to audio interface failed (%s)", snd_strerror(m_ret));
            }
            if ((m_ret = snd_pcm_prepare(m_dev->playback_handle)) < 0) {
                yang_error("cannot prepare audio interface for use (%s)", snd_strerror(m_ret));
            }
        } else {
            yang_error("Couldn't write as many samples as I wanted (%d instead of %d)", m_ret, len);
        }
        return 1;
    }
    return Yang_Ok;
...
int32_t YangAudioAecLinux::alsa_device_capture_ready(struct pollfd *pfds, uint32_t nfds) {
    unsigned short revents = 0;
    if ((m_ret = snd_pcm_poll_descriptors_revents(m_dev->capture_handle, pfds, m_dev->readN, &revents)) < 0) {
        yang_error("error in alsa_device_capture_ready: %s", snd_strerror(m_ret));
        return pfds[0].revents & POLLIN;
    }
    return revents & POLLIN;
}

int32_t YangAudioAecLinux::alsa_device_playback_ready(struct pollfd *pfds, uint32_t nfds) {
    unsigned short revents = 0;
    if ((m_ret = snd_pcm_poll_descriptors_revents(m_dev->playback_handle, pfds + m_dev->readN, m_dev->writeN, &revents)) < 0) {
        yang_error("error in alsa_device_playback_ready: %s", snd_strerror(m_ret));
...
void YangAudioAecLinux::alsa_device_getfds(struct pollfd *pfds, uint32_t nfds) {
    int32_t i;
...
void YangAudioAecLinux::startLoop() {
    ...
    pollfd *pfds = static_cast<pollfd*>(malloc(sizeof(*pfds) * nfds));
    alsa_device_getfds(pfds, nfds);
    int32_t audiolen = m_captureFrames * m_captureChannel * 2;

    std::vector<uint8_t> pcm_write(audiolen, 0);
    uint8_t *tmp = nullptr;
    ...
    while (m_loops) {
        ...
        if (playDeviceState && alsa_device_playback_ready(pfds, nfds)) {
            ...
            alsa_device_write(reinterpret_cast<short*>(tmp), m_captureFrames);
            if (readStart) {
                m_ahandle->putEchoPlay(reinterpret_cast<short*>(tmp), audiolen);
            }
        }
        if (catpureDeviceState && alsa_device_capture_ready(pfds, nfds)) {
            alsa_device_read(reinterpret_cast<short*>(m_buffer), m_captureFrames);
            if (readStart) {
