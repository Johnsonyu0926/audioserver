// Filename: YangAudioCaptureLinux.cpp
// Score: 95
...
int32_t YangAudioCaptureLinux::init() {
    ...
    if ((err = snd_pcm_open(&m_handle, m_avinfo->audio.aIndex == -1 ? "default" : device_name, SND_PCM_STREAM_CAPTURE, 0)) < 0) {
    ...
    if ((err = snd_pcm_hw_params_set_rate_near(m_handle, hw_params, &m_sample, 0)) < 0) {
    ...
    if ((err = snd_pcm_hw_params_set_period_size_near(m_handle, hw_params, &m_frames, &dir)) < 0) {
...
int32_t YangAudioCaptureLinux::alsa_device_capture_ready(struct pollfd *pfds, uint32_t nfds) {
    unsigned short revents = 0;
    int32_t ret = 0;
    if ((ret = snd_pcm_poll_descriptors_revents(m_handle, pfds, m_readN, &revents)) < 0) {
    ...
    if ((ret = snd_pcm_readi(m_handle, pcm, len)) != len) {
        if (ret < 0) {
            if (ret == -EPIPE) {
                ...
                yang_error("read from audio interface failed (%s)", snd_strerror(ret));
            }
            if ((ret = snd_pcm_prepare(m_handle)) < 0) {
                yang_error("cannot prepare audio interface for use (%s)", snd_strerror(ret));
            }
            if ((ret = snd_pcm_start(m_handle)) < 0) {
                yang_error("cannot prepare audio interface for use (%s)", snd_strerror(ret));
            }
        } else {
            yang_error("Couldn't read as many samples as I wanted (%d instead of %d)", ret, len);
        }
        return 1;
    }
    return Yang_Ok;
...
void YangAudioCaptureLinux::startLoop() {
    ...
    if ((err = snd_pcm_poll_descriptors(m_handle, read_fd, m_readN)) != m_readN) {
