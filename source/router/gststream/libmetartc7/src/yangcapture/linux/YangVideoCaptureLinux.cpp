// Filename: YangVideoCaptureLinux.cpp
// Score: 95
    ...
    while ((vet = ioctl(m_vd_id, VIDIOC_ENUM_FMT, &fmt)) != -1) {
        ...
        while (!ioctl(m_vd_id, VIDIOC_ENUM_FRAMESIZES, &frmsize)) {
            if (frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
                ...
                yang_trace("\nSTEPWISE: line: %d %d: {%d*%d}\n", __LINE__, frmsize.index, frmsize.stepwise.max_width, frmsize.stepwise.max_height);
            }
...
int32_t YangVideoCaptureLinux::init() {
    ...
    v4_format.fmt.pix.pixelformat = format;
    ...
    for (i = 0; i < tV4L2_reqbuf.count; i++) {
        ...
        m_user_buffer[i].start = (uint8_t*)mmap(NULL, tV4L2buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, m_vd_id, tV4L2buf.m.offset);
...
long YangVideoCaptureLinux::m_difftime(struct timeval *p_start, struct timeval *p_end) {
    return (p_end->tv_sec - p_start->tv_sec) * 1000000 + (p_end->tv_usec - p_start->tv_usec);
}
