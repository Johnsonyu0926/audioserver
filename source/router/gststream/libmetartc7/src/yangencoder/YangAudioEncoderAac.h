// Filename: YangAudioEncoderAac.h
// Score: 95
...
private:
    ...
    void loadLib();
    void unloadLib();
    faacEncHandle (*yang_faacEncOpen)(unsigned long sampleRate, uint32_t numChannels, unsigned long *inputSamples, unsigned long *maxOutputBytes);
    int32_t (*yang_faacEncSetConfiguration)(faacEncHandle hEncoder, faacEncConfigurationPtr config);
    int32_t (*yang_faacEncEncode)(faacEncHandle hEncoder, int32_t *inputBuffer, uint32_t samplesInput, uint8_t *outputBuffer, uint32_t bufferSize);
    int32_t (*yang_faacEncClose)(faacEncHandle hEncoder);
    faacEncConfigurationPtr (*yang_faacEncGetCurrentConfiguration)(faacEncHandle hEncoder);
};

#endif /* __YangAudioEncoderAac__ */
// By GST @Date