// Filename: YangAudioEncoderMp3.cpp
// Score: 95
...
void YangAudioEncoderMp3::loadLib() {
    ...
    yang_lame_encode_buffer_interleaved = (int32_t (*)(lame_global_flags* gfp, int16_t pcm[], int32_t num_samples, uint8_t* mp3buf, int32_t mp3buf_size)) m_lib.loadFunction("lame_encode_buffer_interleaved");
...
int32_t YangAudioEncoderMp3::encoder(YangFrame* pframe, YangEncoderCallback *pcallback) {
    ...
    m_bufLen += pframe->nb;
    if (m_bufLen < dwRead) return 1;

    memcpy(pWAVBuffer, temp, dwRead);
    dwWrite = yang_lame_encode_buffer_interleaved(gfp, pWAVBuffer, 1152, pMP3Buffer, 0);
...
void YangAudioEncoderMp3::closeMp3() {
    if (gfp != NULL) yang_lame_close(gfp);
    gfp = NULL;
    ret = 2;
}
// By GST @Date