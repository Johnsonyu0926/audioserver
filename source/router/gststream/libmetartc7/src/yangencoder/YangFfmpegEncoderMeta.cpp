// Filename: YangFfmpegEncoderMeta.cpp
// Score: 95
...
enum AVPixelFormat get_hw_format22(AVCodecContext *ctx, const enum AVPixelFormat *pix_fmts) {
...
void YangFfmpegEncoderMeta::yang_getSpsPps(YangH2645Conf *pconf, YangVideoInfo *p_yvp, YangVideoEncInfo *penc) {
#if Yang_Enable_FfmpegSo
    ...
    if (m_encoderType == Yang_VED_H264) {
        if (m_hwType == YangV_Hw_Intel) m_codec = yang_avcodec_find_encoder_by_name("h264_vaapi");
    ...
    m_codecCtx->get_format = get_hw_format22;
    ...
    if ((ret = set_hwframe_ctx(ctxformat, swformat, p_yvp, m_codecCtx, hw_device_ctx, p_yvp->outWidth, p_yvp->outHeight)) < 0) {
...
void YangFfmpegEncoderMeta::yang_initVmd(YangVideoMeta *p_vmd, YangVideoInfo *p_yvp, YangVideoEncInfo *penc) {
    if (!p_vmd->isInit) {
