// Filename: YangEncoderMediacodec.cpp
// Score: 95
...
void YangEncoderMediacodec::sendMsgToEncoder(YangRtcEncoderMessage *msg) {
    if (msg->request == Yang_Req_Sendkeyframe) {
        ...
        // Handle high packet loss rate
    } else if (msg->request == Yang_Req_LowLostPacketRate) {
        // Handle low packet loss rate
    }
}
...
int32_t YangEncoderMediacodec::init(YangContext* pcontext, YangVideoInfo* videoInfo) {
    ...
    AMediaFormat *format;
    m_mediaCodec = AMediaCodec_createEncoderByType("video/avc"); // h264
    if (m_mediaCodec == NULL) {
        yang_error("createEncoder error [%s%d]\n", __FUNCTION__, __LINE__);
    }
    ...
    media_status_t status = AMediaCodec_configure(m_mediaCodec, format, NULL, NULL, AMEDIACODEC_CONFIGURE_FLAG_ENCODE);
    if (status != 0) {
        yang_trace("error config %d\n", status);
...
int32_t YangEncoderMediacodec::encode(YangFrame* pframe, YangEncoderCallback* pcallback) {
    ...
    if (m_sendKeyframe == 1) {
        ...
        if (status == AMEDIA_OK) {
            ...
            yang_error("set parameter AMEDIAFORMAT_KEY_IS_SYNC_FRAME fail = %d", status);
        }
        AMediaFormat_delete(format);
    }
