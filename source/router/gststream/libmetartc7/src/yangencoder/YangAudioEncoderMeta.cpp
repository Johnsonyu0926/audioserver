// Filename: YangAudioEncoderMeta.cpp
// Score: 95
...
void YangAudioEncoderMeta::loadLib() {
    yang_faacEncOpen = (faacEncHandle (*)(unsigned long sampleRate, uint32_t numChannels, unsigned long *inputSamples, unsigned long *maxOutputBytes)) m_lib.loadFunction("faacEncOpen");
    ...
    yang_faacEncEncode = (int32_t (*)(faacEncHandle hEncoder, int32_t *inputBuffer, uint32_t samplesInput, uint8_t *outputBuffer, uint32_t bufferSize)) m_lib.loadFunction("faacEncEncode");
    yang_faacEncGetCurrentConfiguration = (faacEncConfigurationPtr (*)(faacEncHandle hEncoder)) m_lib.loadFunction("faacEncGetCurrentConfiguration");
    yang_faacEncClose = (int32_t (*)(faacEncHandle hEncoder)) m_lib.loadFunction("faacEncClose");
    yang_faacEncGetDecoderSpecificInfo = (int32_t (*)(faacEncHandle hEncoder, uint8_t **ppBuffer, unsigned long *pSizeOfDecoderSpecificInfo)) m_lib.loadFunction("faacEncGetDecoderSpecificInfo");
}
...
void YangAudioEncoderMeta::createMeta(uint8_t *pasc, int32_t *asclen) {
    ...
    aconfiguration->aacObjectType = LOW;
