// Filename: YangFfmpegEncoderMeta.h
// Score: 95
...
private:
    ...
    void yang_getH2645Config(uint8_t *p_configBuf, int32_t *p_configLen, YangVideoInfo *p_config);
    ...
    int32_t (*yang_av_hwdevice_ctx_create)(AVBufferRef **device_ctx, enum AVHWDeviceType type, const char *device, AVDictionary *opts, int32_t flags);
    ...
    int32_t (*yang_av_image_fill_arrays)(uint8_t *dst_data[4], int32_t dst_linesize[4], const uint8_t *src, enum AVPixelFormat pix_fmt, int32_t width, int32_t height, int32_t align);
