// Filename: YangAudioEncoderOpus.cpp
// Score: 95
...
void YangAudioEncoderOpus::loadLib() {
    yang_opus_encoder_create = (OpusEncoder* (*)(opus_int32 Fs, int32_t channels, int32_t application, int32_t *error)) m_lib.loadFunction("opus_encoder_create");
    ...
    yang_opus_encode = (opus_int32 (*)(OpusEncoder *st, const opus_int16 *pcm, int32_t frame_size, uint8_t *data, opus_int32 max_data_bytes)) m_lib.loadFunction("opus_encode");
    yang_opus_encoder_ctl = (int32_t (*)(OpusEncoder *st, int32_t request, ...)) m_lib.loadFunction("opus_encoder_ctl");
    yang_opus_encoder_destroy = (void (*)(OpusEncoder *st)) m_lib.loadFunction("opus_encoder_destroy");
...
void YangAudioEncoderOpus::init(YangAudioInfo *pap) {
    ...
    m_encoder = yang_opus_encoder_create(m_audioInfo.sample, m_audioInfo.channel, OPUS_APPLICATION_VOIP, &err);
    if (err < 0) {
...
void YangAudioEncoderOpus::closeEnc() {
    if (m_encoder) yang_opus_encoder_destroy(m_encoder);
    m_encoder = NULL;
}
// By GST @Date