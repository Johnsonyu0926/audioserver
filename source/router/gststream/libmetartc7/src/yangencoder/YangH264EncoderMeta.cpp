...
void YangH264EncoderMeta::loadLib() {
    ...
    yang_x264_encoder_headers = (int32_t (*)(x264_t*, x264_nal_t **pp_nal, int32_t *pi_nal)) m_lib.loadFunction("x264_encoder_headers");
...
void YangH264EncoderMeta::yang_getSpsPps(YangH2645Conf *p264, YangVideoInfo *config, YangVideoEncInfo *penc) {
    x264_t *p264Handle = NULL;
    x264_param_t *param = new x264_param_t();
    if (penc->preset < 5)
        yang_x264_param_default_preset(param, x264_preset_names[penc->preset], "zerolatency");
    else
...
void YangH264EncoderMeta::yang_initVmd(YangVideoMeta *p_vmd, YangVideoInfo *p_config, YangVideoEncInfo *penc) {
    if (!p_vmd->isInit) {
        yang_getSpsPps(&p_vmd->mp4Meta, p_config, penc);
        yang_getConfig_Flv_H264(&p_vmd->mp4Meta, p_vmd->livingMeta.buffer, &p_vmd->livingMeta.bufLen);
        p_vmd->isInit = 1;
    }
}
// By GST @Date