...
void YangH264EncoderSoft::loadLib() {
    yang_x264_param_default = (void (*)(x264_param_t*)) m_lib.loadFunction("x264_param_default");
    yang_x264_param_default_preset = (int32_t (*)(x264_param_t*, const char *preset, const char *tune)) m_lib.loadFunction("x264_param_default_preset");
    yang_x264_param_apply_profile = (int32_t (*)(x264_param_t*, const char *profile)) m_lib.loadFunction("x264_param_apply_profile");
    char s[30];
    memset(s, 0, 30);
    sprintf(s, "x264_encoder_open_%d", X264_BUILD);
    yang_x264_encoder_open = (x264_t* (*)(x264_param_t*)) m_lib.loadFunction(s);
    yang_x264_picture_alloc = (int32_t (*)(x264_picture_t *pic, int32_t i_csp, int32_t i_width, int32_t i_height)) m_lib.loadFunction("x264_picture_alloc");
    yang_x264_encoder_encode = (int32_t (*)(x264_t*, x264_nal_t **pp_nal, int32_t *pi_nal, x264_picture_t *pic_in, x264_picture_t *pic_out)) m_lib.loadFunction("x264_encoder_encode");
    yang_x264_picture_clean = (void (*)(x264_picture_t *pic)) m_lib.loadFunction("x264_picture_clean");
    yang_x264_encoder_close = (void (*)(x264_t*)) m_lib.loadFunction("x264_encoder_close");
}
...
void YangH264EncoderSoft::sendMsgToEncoder(YangRtcEncoderMessage *msg) {
    if (msg->request == Yang_Req_Sendkeyframe) {
        ...
        // Handle high lost packet rate
    } else if (msg->request == Yang_Req_LowLostPacketRate) {
        // Handle low lost packet rate
    }
}
...
void YangH264EncoderSoft::initX264Param(YangVideoInfo *pvp, YangVideoEncInfo *penc, x264_param_t *param) {
...
int32_t YangH264EncoderSoft::init(YangContext* pcontext, YangVideoInfo* videoInfo) {
    ...
    if (encInfo->preset < 5)
        yang_x264_param_default_preset(param, x264_preset_names[encInfo->preset], "zerolatency");
    else
    ...
    int32_t x264Format = videoInfo->bitDepth == 8 ? X264_CSP_I420 : X264_CSP_HIGH_DEPTH;
...
int32_t YangH264EncoderSoft::encode(YangFrame* pframe, YangEncoderCallback* pcallback) {
    ...
    if (yang_x264_encoder_encode(m_264Handle, &m_264Nal, &m_i264Nal, m_264Pic, &pic_out) < 0) {
