...
void CALLBACK YangTimer::TimeEvent(PVOID user, BOOLEAN TimerOrWaitFired2) {
    YangTimer* timer = static_cast<YangTimer*>(user);
...
YangTimer::YangTimer() : m_loop(0), m_isStart(0), m_waitState(0), m_waitTime(100), m_task(nullptr), m_taskId(0) {
#if Yang_Enable_Timer_Phtread
    ...
    m_winEvent = CreateEvent(nullptr, TRUE, FALSE, nullptr);
#else
...
void YangTimer::startLoop() {
...
#else
#if Yang_OS_WIN
    startWindowsEventTime(m_waitTime, reinterpret_cast<DWORD_PTR>(this));
    ...
    int ret = timerfd_settime(m_timerfd, TFD_TIMER_ABSTIME, &itimer, nullptr);
    if (ret == -1) {
        yang_error("timerfd_settime");
    }

    int opts = fcntl(m_timerfd, F_GETFL);
    if (opts < 0) {
