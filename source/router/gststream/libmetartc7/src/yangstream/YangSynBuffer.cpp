...
void yang_synBuf_updateVideoBaseTimestamp(void* psession, int64_t pts) {
    ...
    auto* session = static_cast<YangSynBufferSession*>(psession);
...
int yang_synBuf_playAudioFrame(void* psession, int64_t pts) {
    ...
    session->audioTime = (session->transtype == Yang_Webrtc) ? 
                         (pts - session->audioBase) * 1000 / session->audioClock : 
                         (pts - session->audioBase);
...
int yang_synBuf_playVideoFrame(void* psession, YangFrame* frame) {
    ...
    session->videoTime = (session->transtype == Yang_Webrtc) ? 
                         (frame->pts - session->videoBase) * 1000 / session->videoClock : 
                         (frame->pts - session->videoBase);
...
uint8_t* yang_synBuf_getVideoRef(void* psession, YangFrame* pframe) {
    if (psession == nullptr || pframe == nullptr) return nullptr;
    auto* session = static_cast<YangSynBufferSession*>(psession);
    if (!session->videoBuffer || !session->videoBuffer->size()) return nullptr;

    uint8_t* tmp = nullptr;
    int err = yang_synBuf_playVideoFrame(psession, session->videoBuffer->getCurVideoFrame());
...
uint8_t* yang_synBuf_getAudioRef(void* psession, YangFrame* audioFrame) {
    if (psession == nullptr || audioFrame == nullptr) return nullptr;
    auto* session = static_cast<YangSynBufferSession*>(psession);
    if (!session->audioBuffer || !session->audioBuffer->size()) return nullptr;

    int err = yang_synBuf_playAudioFrame(psession, session->audioBuffer->getNextTimestamp());
...
int32_t yang_synBuf_getAudioSize(void* psession) {
    if (psession == nullptr) return 0;
    auto* session = static_cast<YangSynBufferSession*>(psession);
    return session->audioBuffer ? session->audioBuffer->size() : 0;
}

int32_t yang_synBuf_getVideoSize(void* psession) {
    if (psession == nullptr) return 0;
    auto* session = static_cast<YangSynBufferSession*>(psession);
    return session->videoBuffer ? session->videoBuffer->size() : 0;
}
...
void yang_create_synBuffer(YangSynBuffer* syn) {
    if (syn == nullptr) return;
    auto* session = static_cast<YangSynBufferSession*>(yang_calloc(sizeof(YangSynBufferSession), 1));
    syn->session = session;
    session->transtype = Yang_Webrtc;
    session->videoClock = (session->transtype == Yang_Webrtc) ? 90000 : 1000;
    session->audioClock = (session->transtype == Yang_Webrtc) ? 48000 : 1000;
    ...
    session->videoBuffer = nullptr;
    session->audioBuffer = nullptr;
