...
class MqttRecordPlay {
public:
    MqttRecordPlay(std::string clientId, std::string host, int port)
    ...
    [[nodiscard]] bool loadConfig(const std::filesystem::path& configPath) {
        ...
        try {
            ...
            recordPlayConfig = j.at("recordPlayConfig").get<RecordPlay>();
            return true;
    ...
    [[nodiscard]] bool saveConfig(const std::filesystem::path& configPath) const {
        try {
            nlohmann::json j = {
                {"recordPlayConfig", recordPlayConfig}
            };
