...
class MqttTtsPlay {
public:
    MqttTtsPlay(std::string clientId, std::string host, int port)
    ...
    [[nodiscard]] bool loadConfig(const std::filesystem::path& configPath) {
        ...
        try {
            ...
            ttsConfig = j.at("ttsConfig").get<TtsPlay>();
            return true;
    ...
    [[nodiscard]] bool playTts(std::string_view text) {
        return ttsConfig.play(text);
    }
