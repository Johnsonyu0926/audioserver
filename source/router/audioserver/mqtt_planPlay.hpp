...
#include "mqtt.hpp"
#include "planPlay.hpp"

class MqttPlanPlay {
public:
    MqttPlanPlay(std::string clientId, std::string host, int port)
    ...
    [[nodiscard]] bool loadConfig(const std::filesystem::path& configPath) {
        ...
        try {
            ...
            planPlayConfig = j.at("planPlayConfig").get<PlanPlay>();
            return true;
    ...
    [[nodiscard]] nlohmann::json handlePlanPlayCommand(const nlohmann::json& command) {
        // 这里应该实现实际的计划播放逻辑
