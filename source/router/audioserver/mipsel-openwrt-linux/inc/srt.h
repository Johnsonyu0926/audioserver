...
#define EZ_SRT_VER "1.1.2.211027"
...
typedef enum SRT_SOCKOPT {
   ...
   SRTO_BINDTODEVICE         // Set bind to special network card
} SRT_SOCKOPT;
...
SRT_API       int srt_bind3        (SRTSOCKET u, UDPSOCKET sys_udp_sock, struct sockaddr* addr, int addrlen);
...
// added by zhanglei   20160216
// libCASClient can use this interface to control UDT to or not to receive data from UDP socket buffer
SRT_API void srt_setrecvavail(int sock, SRTSOCKET u, bool bRecv);
