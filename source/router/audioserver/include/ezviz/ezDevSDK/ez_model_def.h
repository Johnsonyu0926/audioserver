...
#define EZ_DOMAIN_LEN          64
#define EZ_IDENTIFIER_LEN      64
...
/**
 * \brief   ez_msg_source_type 通道消息来源
 * \warning 
 * \note    
 */
typedef enum
{
    ez_msg_source_microkernel,
    ez_msg_source_ezlink
} ez_msg_source_type;
...
{   
    ...
    int msg_source_type;                        ///< ez_msg_source_type
    ezdev_sdk_kernel_context_t mul_context;             //多中心指针
}ez_basic_info;  
...
typedef struct
{
    ...
    unsigned int msg_seq_index;     ///< 消息的 seq_index（仅用于批量指令，单条指令默认为0）
...
/**
 * \brief   消息json类型
 * \warning 内部根据传入的类型,将传入内容转成对一个json类型
 * \note      
 */
typedef  enum
{
...
typedef struct
{
    ...
    char identifier[EZ_COMMON_LEN];
    int (*das_req_router)   (ez_basic_info* basic_info, char* msg_type,                      ez_model_msg* buf, unsigned int msg_seq, unsigned int msg_seq_index);
    int (*das_reply_router) (ez_basic_info* basic_info, char* msg_type, ez_err_info* status, ez_model_msg* buf, unsigned int msg_seq, unsigned int msg_seq_index);
    int mulitple_data_timeout_interval;
} ez_domain_reg;
...
typedef struct
{
    void (*ez_model_default_cb)(ez_basic_info* basic_info, char* msg_type, void* buf, int len, unsigned int msg_seq, unsigned int msg_seq_index);
    int mulitple_data_timeout_interval;
}ez_model_default_cb;
